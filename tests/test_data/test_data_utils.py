...
class TestDataUtils(TestCase):
    def test_pseudo_collate(self):
        ...
        data_batch = [dict(inputs=input1, data_sample=label1), dict(inputs=input2, data_sample=label2)]
        ...
        data_batch = [
            (
                dict(inputs=input1, data_sample=data_sample1, value=1, name="1", nested=dict(data_sample=data_sample1)),
                dict(inputs=input2, data_sample=data_sample2, value=2, name="2", nested=dict(data_sample=data_sample2)),
            ),
            (
                dict(inputs=input1, data_sample=data_sample1, value=1, name="1", nested=dict(data_sample=data_sample1)),
                dict(inputs=input2, data_sample=data_sample2, value=2, name="2", nested=dict(data_sample=data_sample2)),
    ...
    def test_default_collate(self):
        ...
        self.assertTrue(torch.allclose(batch_inputs_0, torch.stack([input1, input1])))
        self.assertTrue(torch.allclose(batch_inputs_1, torch.stack([input2, input2])))

        target1 = torch.stack([torch.tensor(1), torch.tensor(1)])
        target2 = torch.stack([torch.tensor(2), torch.tensor(2)])

        self.assertTrue(torch.allclose(batch_value_0.to(target1.dtype), target1))
        self.assertTrue(torch.allclose(batch_value_1.to(target2.dtype), target2))

        self.assertTrue(torch.allclose(batch_array_0.to(target1.dtype), target1))
        self.assertTrue(torch.allclose(batch_array_1.to(target2.dtype), target2))
