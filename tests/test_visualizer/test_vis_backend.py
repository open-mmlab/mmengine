...
class TestLocalVisBackend:
    def test_init(self):
        ...
        local_vis_backend = VISBACKENDS.build(dict(type="LocalVisBackend", save_dir="temp_dir"))
    ...
    def test_add_image(self):
        ...
        assert os.path.exists(os.path.join(local_vis_backend._img_save_dir, "img_0.png"))
        local_vis_backend.add_image("img", image.astype(np.uint8), step=2)
        assert os.path.exists(os.path.join(local_vis_backend._img_save_dir, "img_2.png"))
        shutil.rmtree("temp_dir")
    ...
    def test_add_scalar(self):
        ...
        local_vis_backend = LocalVisBackend("temp_dir")
        local_vis_backend.add_scalar("map", torch.tensor(1.0))
    ...
    def test_add_scalars(self):
        ...
        assert os.path.exists(local_vis_backend._scalar_save_file)
        assert os.path.exists(os.path.join(local_vis_backend._save_dir, "temp.json"))
...
class TestTensorboardVisBackend:
    ...
    def test_init(self):
        ...
        VISBACKENDS.build(dict(type="TensorboardVisBackend", save_dir="temp_dir"))

    def test_experiment(self):
        tensorboard_vis_backend = TensorboardVisBackend("temp_dir")
        assert tensorboard_vis_backend.experiment == tensorboard_vis_backend._tensorboard
        shutil.rmtree("temp_dir")
    ...
    def test_add_scalar(self):
        ...
        with warnings.catch_warnings(record=True) as record:
            ...
            tensorboard_vis_backend.add_scalar("map", np.array([95])[0], step=1)
        assert len(record) == 0
        # test with tensor
    ...
    def test_add_scalars(self):
        ...
        with pytest.raises(AssertionError):
            tensorboard_vis_backend.add_scalars({"map": 0.7, "acc": 0.9, "step": 1})
...
class TestWandbVisBackend:
    ...
    def test_define_metric_cfg(self):
        ...
        wandb_vis_backend = WandbVisBackend("temp_dir", define_metric_cfg=define_metric_cfg)
        wandb_vis_backend._init_env()
        wandb_vis_backend._wandb.define_metric.assert_any_call(name="test1", step_metric="iter")
        wandb_vis_backend._wandb.define_metric.assert_any_call(name="test2", step_metric="epoch")

        # dict
        define_metric_cfg = dict(test3="max")
        wandb_vis_backend = WandbVisBackend("temp_dir", define_metric_cfg=define_metric_cfg)
        wandb_vis_backend._init_env()
        wandb_vis_backend._wandb.define_metric.assert_any_call("test3", summary="max")
...
class TestMLflowVisBackend:
    ...
    def test_create_experiment(self):
        with patch("mlflow.create_experiment") as mock_create_experiment:
            MLflowVisBackend("temp_dir", exp_name="test", artifact_location="foo")._init_env()
            mock_create_experiment.assert_any_call("test", artifact_location="foo")
...
@pytest.mark.skipif(not is_installed("dvclive"), reason="DVCLive is not installed.")
...
@pytest.mark.skipif(platform.system() == "Windows", reason="Aim does not support Windows for now.")
@pytest.mark.skipif(not is_installed("aim"), reason="Aim is not installed.")
