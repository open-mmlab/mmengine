...
class DetDataSample(BaseDataElement):
    ...
    def gt_instances(self, value):
        self.set_field(value=value, name="_gt_instances", dtype=BaseDataElement)
    ...
    def pred_instances(self, value):
        self.set_field(value=value, name="_pred_instances", dtype=BaseDataElement)
...
class TestBaseDataElement(TestCase):
    def setup_data(self):
        ...
        gt_instances = BaseDataElement(bboxes=torch.rand((5, 4)), labels=torch.rand((5,)))
        pred_instances = BaseDataElement(bboxes=torch.rand((5, 4)), scores=torch.rand((5,)))
        data = dict(gt_instances=gt_instances, pred_instances=pred_instances)
        return metainfo, data

    def is_equal(self, x, y):
        assert type(x) is type(y)
        if isinstance(x, int | float | str | list | tuple | dict | set | BaseDataElement):
            return x == y
    ...
    def test_new(self):
        ...
        assert not self.is_equal(new_instances.gt_instances, instances.gt_instances)
    ...
    def test_update(self):
        metainfo, data = self.setup_data()
        instances = BaseDataElement(metainfo=metainfo, **data)
        proposals = BaseDataElement(bboxes=torch.rand((5, 4)), scores=torch.rand((5,)))
        new_instances = BaseDataElement(proposals=proposals)
        instances.update(new_instances)
        self.check_key_value(instances, metainfo, data.update(dict(proposals=proposals)))
    ...
    def test_delete_modify(self):
        ...
        while True:
            if new_metainfo["img_id"] == metainfo["img_id"] or new_metainfo["img_shape"] == metainfo["img_shape"]:
                new_metainfo, new_data = self.setup_data()
            else:
                break
        ...
        assert not self.is_equal(instances.pred_instances, data["pred_instances"])
        ...
        assert not self.is_equal(instances.pop("pred_instances", None), data["pred_instances"])
        with self.assertRaises(AttributeError):
    ...
    def test_repr(self):
        ...
        gt_instances = BaseDataElement(metainfo=metainfo, det_labels=torch.LongTensor([0, 1, 2, 3]))
    ...
    def test_values(self):
        ...
        # test_all_values
        assert len(instances.all_values()) == len(metainfo.values()) + len(data.values())
    ...
    def test_keys(self):
        ...
        # test_all_keys
        assert len(instances.all_keys()) == len(data.keys()) + len(metainfo.keys())
    ...
    def test_items(self):
        ...
        metainfo, data = self.setup_data()
        instances = BaseDataElement(metainfo=metainfo, **data)
        assert len(dict(instances.metainfo_items())) == len(dict(metainfo.items()))
        # test_all_items
        assert len(dict(instances.all_items())) == len(dict(metainfo.items())) + len(dict(data.items()))
