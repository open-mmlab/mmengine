...
class TestLabelData(TestCase):
    def test_label_to_onehot(self):
        ...
        with self.assertRaises(AssertionError):
            LabelData.label_to_onehot(torch.tensor([11], dtype=torch.int64), num_classes)
        onehot = LabelData.label_to_onehot(label=torch.tensor([], dtype=torch.int64), num_classes=num_classes)
        assert (onehot == torch.zeros((num_classes,), dtype=torch.int64)).all()

    def test_onehot_to_label(self):
        # item is not onehot
        with self.assertRaisesRegex(ValueError, "input is not one-hot and can not convert to label"):
            LabelData.onehot_to_label(onehot=torch.tensor([2], dtype=torch.int64))
