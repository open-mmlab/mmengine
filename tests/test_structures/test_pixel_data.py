...
class TestPixelData(TestCase):
    ...
    def test_getitem(self):
        ...
        with pytest.raises(TypeError, match=f"Unsupported type {type(item)} for slicing PixelData"):
            pixel_data[item]
        item = 1
        with pytest.raises(TypeError, match=f"Unsupported type {type(item)} for slicing PixelData"):
            pixel_data[item]
        item = (5.5, 5)
        with pytest.raises(
            TypeError, match=(f"The type of element in input must be int or slice, but got {type(item[0])}")
        ):
            pixel_data[item]
