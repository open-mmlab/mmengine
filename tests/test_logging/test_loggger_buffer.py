...
from mmengine import (CurrentLogBuffer, MinLogBuffer, MaxLogBuffer,
    MeanLogBuffer)
...
class TestLoggerBuffer:
    def test_init(self):
        ...
        log_buffer = CurrentLogBuffer(logs, counts)
    ...
    def test_update(self, array_method):
        ...
        log_buffer = CurrentLogBuffer(max_array, max_count)
        log_buffer.update(1)
        log_history, count_history = log_buffer.data
    ...
    @pytest.mark.parametrize('statistics_method, log_buffer_type',
                             [(np.min, MinLogBuffer), (np.max, MaxLogBuffer)]
                             )
