...
class TestMessageHub:
    def test_init(self):
        ...
        # `Resumed_keys`
        with pytest.raises(AssertionError):
            MessageHub("hello", runtime_info=OrderedDict(iter=1), resumed_keys=OrderedDict(iters=False))
    ...
    def test_get_scalar(self):
        ...
        recorded_history, recorded_count = message_hub.get_scalar("test_value").data
    ...
    def test_get_scalars(self):
        ...
        message_hub = MessageHub.get_instance("mmengine")
        log_dict = dict(loss=1, loss_cls=torch.tensor(2), loss_bbox=np.array(3), loss_iou=dict(value=1, count=2))
    ...
    def test_state_dict(self):
        ...
        assert state_dict["log_scalars"]["loss"].data == (np.array([0.1]), np.array([1]))
    ...
    def test_load_state_dict(self, capsys):
        ...
        assert message_hub2.get_scalar("loss").data == (np.array([0.1]), np.array([1]))
        ...
        assert message_hub3.get_scalar("loss").data == (np.array([0.1]), np.array([1]))
        ...
        state_dict["resumed_keys"] = dict(a=True, b=True, c=True, e=False, f=True)

        message_hub4 = MessageHub.get_instance("test_load_state_dict4")
        message_hub4.load_state_dict(state_dict)
        assert "a" not in message_hub4.log_scalars and "b" in message_hub4.log_scalars
        assert "c" in message_hub4.runtime_info and state_dict["runtime_info"]["d"] is message_hub4.runtime_info["d"]
        assert message_hub4._resumed_keys == OrderedDict(b=True, c=True, e=False)
