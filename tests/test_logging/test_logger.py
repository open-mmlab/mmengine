...
class TestLogger:
    ...
    def test_init_rank0(self, tmp_path):
        ...
        logger = MMLogger.get_instance("rank0.pkg2", log_level="INFO", log_file=str(tmp_file))
        ...
        logger = MMLogger.get_instance("rank0.pkg3", logger_name="logger_test", log_level="INFO")
    ...
    def test_init_rank1(self, tmp_path):
        ...
        logger = MMLogger.get_instance("rank1.pkg2", log_level="INFO", log_file=str(tmp_file))
        assert len(logger.handlers) == 1
        logger = MMLogger.get_instance("rank1.pkg3", log_level="INFO", log_file=str(tmp_file), distributed=True)
    ...
    def test_handler(self, capsys, tmp_path, log_level):
        ...
        match = re.fullmatch(self.stream_handler_regex_time + f" - mmengine - (.*){loglevl_name}(.*) - welcome\n", out)
        assert match is not None

        # test file_handler output plain text without color.
        tmp_file = tmp_path / "tmp_file.log"
        instance_name = f"test_file_{log_level}"
        logger = MMLogger.get_instance(instance_name, log_level=log_level, log_file=tmp_file)
        logger.log(level=log_level, msg="welcome")

        with open(tmp_file) as f:
            log_text = f.read()
            match = re.fullmatch(self.file_handler_regex_time + f" - mmengine - {loglevl_name} - welcome\n", log_text)
            assert match is not None
        # `FileHandler` should be closed in Windows, otherwise we cannot
        # delete the temporary directory
        logging.shutdown()
    ...
    def test_error_format(self, capsys):
        ...
        pattern = (
            self.stream_handler_regex_time + r" - mmengine - (.*)ERROR(.*) - "
    ...
    def test_print_log(self, capsys, tmp_path):
        ...
        out, _ = capsys.readouterr()
        match = re.fullmatch(self.stream_handler_regex_time + " - mmengine - (.*)INFO(.*) - welcome\n", out)
        assert match is not None
    ...
    def test_file_handlers(self, tmp_path):
        tmp_file = tmp_path / "tmp_file.log"
        fh = None
        logger = MMLogger(name="test_file_handlers", log_file=tmp_file, file_handler_cfg=fh)
        assert isinstance(logger.handlers[-1], logging.FileHandler)
        fh = dict(type="BaseRotatingHandler", mode="a")
        logger = MMLogger(name="test_file_handlers", log_file=tmp_file, file_handler_cfg=fh)
        assert isinstance(logger.handlers[-1], logging.handlers.BaseRotatingHandler)
        ...
        logger = MMLogger(name="test_file_handlers", log_file=tmp_file, file_handler_cfg=fh)
        assert isinstance(logger.handlers[-1], logging.handlers.WatchedFileHandler)
        # `FileHandler` should be closed in Windows, otherwise we cannot
        # delete the temporary directory
        logging.shutdown()
        MMLogger._instance_dict.clear()
...
def test_get_device_id():
    ...
    with patch("torch.cuda.is_available", lambda: False), patch_env(None, "0,1,2,3"):
        assert _get_device_id() == 0

    # cuda is not available and local_rank is set
    with patch("torch.cuda.is_available", lambda: False), patch_env("1", "0,1,2,3"):
        assert _get_device_id() == 1

    # CUDA_VISIBLE_DEVICES will not influence non-cuda device
    with patch("torch.cuda.is_available", lambda: False), patch_env("1", "0,100,2,3"):
        assert _get_device_id() == 1

    # cuda is available and local_rank is not set
    with patch("torch.cuda.is_available", lambda: True), patch_env(None, "0,1,2,3"):
        assert _get_device_id() == 0

    # cuda is available and local_rank is set
    with patch("torch.cuda.is_available", lambda: True), patch_env("2", "0,1,2,3"):
        assert _get_device_id() == 2

    # CUDA_VISIBLE_DEVICES worked
    with patch("torch.cuda.is_available", lambda: True), patch_env("2", "0,1,3,5"):
