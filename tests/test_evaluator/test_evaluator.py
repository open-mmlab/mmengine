...
class ToyMetric(BaseMetric):
    ...
    def __init__(self, collect_device: str = "cpu", prefix: Optional[str] = None, dummy_metrics: Optional[dict] = None):
        super().__init__(collect_device=collect_device, prefix=prefix)
        self.dummy_metrics = dummy_metrics

    def process(self, data_batch, predictions):
        results = [{"pred": prediction["label"], "label": prediction["label"]} for prediction in predictions]
...
def generate_test_results(size, batch_size, pred, label):
    ...
    for i in range(num_batch):
        ...
        predictions = [BaseDataElement(pred=pred, label=label) for _ in range(bs)]
...
class TestEvaluator(TestCase):
    def test_single_metric(self):
        ...
        for data_samples, outputs in generate_test_results(size, batch_size, pred=1, label=1):
    ...
    def test_composed_metrics(self):
        cfg = [dict(type="ToyMetric"), dict(type="ToyMetric", dummy_metrics=dict(mAP=0.0))]
    ...
    def test_ambiguous_metric(self):
        cfg = [dict(type="ToyMetric", dummy_metrics=dict(mAP=0.0)), dict(type="ToyMetric", dummy_metrics=dict(mAP=0.0))]
        ...
        with self.assertRaisesRegex(ValueError, "There are multiple evaluation results with the same metric name"):
    ...
    def test_collect_device(self):
        cfg = [
            ...
            dict(type="ToyMetric", collect_device="gpu", dummy_metrics=dict(mAP=0.0)),
        ]
    ...
    def test_offline_evaluate(self):
        ...
        all_predictions = [BaseDataElement(pred=0, label=1) for _ in range(size)]
        ...
        with self.assertRaisesRegex(AssertionError, "data_samples and data should have the same length"):
            evaluator.offline_evaluate(all_predictions, all_data)
    ...
    def test_evaluate_cast_cpu(self):
        ...
        all_data = [
            dict(
                ...
                data_sample=BaseDataElement(label=torch.ones((1,), device="cuda")),
            )
            for _ in range(size)
        ]
        all_predictions = [
            BaseDataElement(pred=torch.zeros((1,), device="cuda"), label=torch.ones((1,), device="cuda"))
            for _ in range(size)
        ]
        for data, pred in zip(all_data, all_predictions, strict=False):
