...
def test_get_state_dict():
    ...
    if torch.__version__ != "parrots":
        assert_tensor_equal(
            state_dict["block.norm.num_batches_tracked"], wrapped_model.module.block.norm.num_batches_tracked
        )
    ...
    if torch.__version__ != "parrots":
        assert_tensor_equal(
            state_dict["block.norm.num_batches_tracked"], wrapped_model.module.block.module.norm.num_batches_tracked
        )
...
def test_checkpoint_loader():
    ...
    for filename, fn_name in zip(filenames, fn_names, strict=False):
...
def test_save_checkpoint(tmp_path):
    ...
    save_checkpoint(model.state_dict(), filename, backend_args={"backend": "local"})

    filename = str(tmp_path / "checkpoint4.pth")
    save_checkpoint(model.state_dict(), filename, file_client_args={"backend": "disk"})
    ...
    with patch.object(PetrelBackend, "put") as mock_method:
        ...
        save_checkpoint(model.state_dict(), filename, file_client_args={"backend": "petrel"})
    mock_method.assert_called()
...
def test_load_from_local():
    ...
    assert_tensor_equal(checkpoint["block.conv.weight"], model.block.conv.weight)
    os.remove(checkpoint_path)
