...
from mmengine.optim import DefaultOptimWrapperConstructor, MultiStepLR, OptimWrapper, OptimWrapperDict, StepLR
...
from mmengine.runner import BaseLoop, EpochBasedTrainLoop, IterBasedTrainLoop, LogProcessor, Runner, TestLoop, ValLoop
...
class ToyMultipleOptimizerConstructor:
    def __init__(self, optim_wrapper_cfg, paramwise_cfg=None):
        if not isinstance(optim_wrapper_cfg, dict):
            raise TypeError("optimizer_cfg should be a dict", f"but got {type(optim_wrapper_cfg)}")
        assert paramwise_cfg is None, "parawise_cfg should be set in each optimizer separately"
        ...
        for key, cfg in self.optim_wrapper_cfg.items():
            ...
            self.constructors[key] = DefaultOptimWrapperConstructor(_cfg, paramwise_cfg_)
...
class TestRunner(TestCase):
    def setUp(self):
        ...
        OPTIM_WRAPPER_CONSTRUCTORS.register_module(module=ToyMultipleOptimizerConstructor, force=True)
        ...
        epoch_based_cfg = dict(
            ...
            optim_wrapper=dict(type="OptimWrapper", optimizer=dict(type="SGD", lr=0.01)),
            param_scheduler=dict(type="MultiStepLR", milestones=[1, 2]),
            val_evaluator=dict(type="ToyMetric1"),
            test_evaluator=dict(type="ToyMetric1"),
            train_cfg=dict(by_epoch=True, max_epochs=3, val_interval=1, val_begin=1),
            val_cfg=dict(),
            test_cfg=dict(),
            custom_hooks=[],
            default_hooks=dict(
    ...
    def tearDown(self):
        ...
        OPTIM_WRAPPER_CONSTRUCTORS.module_dict.pop("ToyMultipleOptimizerConstructor")
    ...
    def test_init(self):
        ...
        self.assertIsInstance(runner.model, (nn.Module, DistributedDataParallel))
        ...
        cfg.param_scheduler = [dict(type="MultiStepLR", milestones=[1, 2]), dict(type="MultiStepLR", milestones=[2, 3])]
        ...
        cfg.param_scheduler = MultiStepLR(milestones=[1, 2], optimizer=optimizer)
        ...
        with self.assertRaisesRegex(TypeError, "`param_scheduler` should be a"):
            Runner(**cfg)
    ...
    def test_dump_config(self):
        ...
        for idx, cfg in enumerate((cfg, cfg._cfg_dict)):
            ...
            with tempfile.TemporaryDirectory() as temp_config_dir:
                ...
                file_cfg = Config(self.epoch_based_cfg._cfg_dict, filename=temp_config_file.name)
                file_cfg.experiment_name = f"test_dump2{idx}"
                runner = Runner.from_cfg(cfg=file_cfg)
                assert osp.exists(osp.join(runner.work_dir, osp.basename(temp_config_file.name)))
    ...
    def test_build_message_hub(self):
        ...
        runner = Runner.from_cfg(self.epoch_based_cfg)
        self.assertIsInstance(runner.message_hub, MessageHub)
        self.assertEqual(runner.message_hub.instance_name, runner.experiment_name)
    ...
    def test_build_visualizer(self):
        ...
        runner = Runner.from_cfg(self.epoch_based_cfg)
        self.assertIsInstance(runner.visualizer, Visualizer)
        self.assertEqual(runner.experiment_name, runner.visualizer.instance_name)

        # input is a Visualizer object
        self.assertEqual(id(runner.build_visualizer(runner.visualizer)), id(runner.visualizer))
    ...
    def test_default_scope(self):
        TOY_SCHEDULERS = Registry("parameter scheduler", parent=PARAM_SCHEDULERS, scope="toy")
        ...
        self.epoch_based_cfg.param_scheduler = dict(type="ToyScheduler", milestones=[1, 2])
    ...
    def test_build_model(self):
        ...
        self.assertIsInstance(runner.model.data_preprocessor, BaseDataPreprocessor)
        ...
        # in model config.
        self.assertIsInstance(runner.model.data_preprocessor, ImgDataPreprocessor)
    ...
    def test_scale_lr(self):
        ...
        optim_wrapper = OptimWrapper(SGD(runner.model.parameters(), lr=0.01))
        runner.scale_lr(optim_wrapper, auto_scale_lr)
        self.assertEqual(optim_wrapper.optimizer.param_groups[0]["lr"], 0.01 * (real_bs / 16))
    ...
    def test_build_optim_wrapper(self):
        ...
        # 1.2 input is a dict
        optim_wrapper = runner.build_optim_wrapper(dict(type="OptimWrapper", optimizer=dict(type="SGD", lr=0.01)))
        self.assertIsInstance(optim_wrapper, OptimWrapper)

        # 1.3 use default OptimWrapper type.
        optim_wrapper = runner.build_optim_wrapper(dict(optimizer=dict(type="SGD", lr=0.01)))
        self.assertIsInstance(optim_wrapper, OptimWrapper)
        ...
        optimizer1 = SGD(runner.model.linear1.parameters(), lr=0.01)
        optim_wrapper1 = OptimWrapper(optimizer1)
        optim_wrapper2 = dict(type="OptimWrapper", optimizer=dict(type="Adam", lr=0.01))
        optim_cfg = dict(key1=optim_wrapper1, key2=optim_wrapper2)
        with self.assertRaisesRegex(ValueError, "each item mush be an optimizer object"):
            runner.build_optim_wrapper(optim_cfg)

        # 2.3 input is a dict which contains multiple configs
        optim_wrapper_cfg = dict(
            linear1=dict(type="OptimWrapper", optimizer=dict(type="SGD", lr=0.01)),
            linear2=dict(type="OptimWrapper", optimizer=dict(type="Adam", lr=0.02)),
        ...
        model = nn.Linear(1, 1)
        optimizer = SGD(model.parameters(), lr=0.1)
        optim_wrapper_cfg = dict(optimizer=optimizer, type="ToyOptimWrapper", accumulative_counts=2)
        optim_wrapper = runner.build_optim_wrapper(optim_wrapper_cfg)
    ...
    def test_build_param_scheduler(self):
        ...
        cfg = [dict(type="MultiStepLR", milestones=[1, 2]), dict(type="StepLR", step_size=1)]
        param_schedulers = runner.build_param_scheduler(cfg)
        ...
        cfg = dict(
            ...
            key2=[dict(type="MultiStepLR", milestones=[1, 2]), dict(type="StepLR", step_size=1)],
        )
        param_schedulers = runner.build_param_scheduler(cfg)
        self.assertIsInstance(param_schedulers, dict)
        self.assertEqual(len(param_schedulers), 2)
    ...
    def test_build_evaluator(self):
        ...
        # test collect device
        evaluator = [dict(type="ToyMetric1", collect_device="cpu"), dict(type="ToyMetric2", collect_device="gpu")]
        ...
        evaluator = dict(
            type="ToyEvaluator",
            metrics=[dict(type="ToyMetric1", collect_device="cpu"), dict(type="ToyMetric2", collect_device="gpu")],
        )
        _evaluator = runner.build_evaluator(evaluator)
    ...
    def test_build_dataloader(self):
        ...
        dataloader = runner.build_dataloader(cfg, seed=seed, diff_rank_seed=True)
        ...
        with self.assertRaisesRegex(TypeError, "collate_fn should be a dict or callable object"):
            cfg = dict(
                dataset=dict(type="ToyDataset"),
                sampler=dict(type="DefaultSampler", shuffle=True),
                worker_init_fn=dict(type="custom_worker_init"),
    ...
    def test_train(self):
        ...
        batch_idx_results = []
        batch_idx_targets = list(range(4)) * 3  # train and val
        val_epoch_results = []
        ...
        for (
            result,
            target,
        ) in zip(epoch_results, epoch_targets, strict=False):
            self.assertEqual(result, target)
        for (
            result,
            target,
        ) in zip(iter_results, iter_targets, strict=False):
            self.assertEqual(result, target)
        for (
            result,
            target,
        ) in zip(batch_idx_results, batch_idx_targets, strict=False):
            self.assertEqual(result, target)
        for (
            result,
            target,
        ) in zip(val_epoch_results, val_epoch_targets, strict=False):
            self.assertEqual(result, target)
        ...
        cfg.train_cfg = dict(by_epoch=False, max_iters=12, val_interval=4, val_begin=4)
        runner = Runner.from_cfg(cfg)
        runner.train()
        ...
        for (
            result,
            target,
        ) in zip(val_iter_results, val_iter_targets, strict=False):
            self.assertEqual(result, target)
        for (
            result,
            target,
        ) in zip(val_batch_idx_results, val_batch_idx_targets, strict=False):
            self.assertEqual(result, target)
        ...
        assert isinstance(runner.train_loop, IterBasedTrainLoop)
        assert isinstance(runner.train_loop.dataloader_iterator, _InfiniteDataloaderIterator)
        ...
        cfg.train_dataloader.sampler = dict(type="DefaultSampler", shuffle=True)
        cfg.custom_hooks = [dict(type="TestIterDynamicIntervalHook", priority=50)]
        cfg.train_cfg = dict(
            by_epoch=False, max_iters=max_iters, val_interval=interval, dynamic_intervals=dynamic_intervals
        )
        runner = Runner.from_cfg(cfg)
        runner.train()
        for (
        ...
        for (
            result,
            target,
        ) in zip(val_interval_results, val_interval_targets, strict=False):
            self.assertEqual(result, target)
        ...
        cfg.train_dataloader.sampler = dict(type="DefaultSampler", shuffle=True)
        cfg.custom_hooks = [dict(type="TestEpochDynamicIntervalHook", priority=50)]
        cfg.train_cfg = dict(
            by_epoch=True, max_epochs=max_epochs, val_interval=interval, dynamic_intervals=dynamic_intervals
        )
        runner = Runner.from_cfg(cfg)
        runner.train()
        for (
        ...
        cfg.train_dataloader.update(collate_fn=dict(type="custom_collate", pad_value=100))
        runner = Runner.from_cfg(cfg)
        runner.train()
        ...
        cfg.train_dataloader.update(worker_init_fn=dict(type="custom_worker_init"))
        runner = Runner.from_cfg(cfg)
        runner.train()
    ...
    @skipIf(SKIP_TEST_COMPILE, reason="torch.compile is not valid, please install PyTorch>=2.0.0")
    def test_train_with_compile(self):
        ...
        with open(osp.join(f"{runner.log_dir}", f"{runner.timestamp}.log")) as f:
    ...
    def test_register_hook(self):
        ...
        with self.assertRaisesRegex(TypeError, "hook should be an instance of Hook or dict"):
        ...
        self.assertEqual(get_priority(runner._hooks[0].priority), get_priority("NORMAL"))
        ...
        self.assertEqual(len(runner._hooks), 1)
        self.assertTrue(isinstance(runner._hooks[0], IterTimerHook))
        self.assertEqual(get_priority(runner._hooks[0].priority), get_priority("BELOW_NORMAL"))
        ...
        self.assertEqual(get_priority(runner._hooks[0].priority), get_priority("VERY_HIGH"))
        ...
        self.assertEqual(get_priority(runner._hooks[2].priority), get_priority("VERY_LOW"))
        ...
        self.assertEqual(get_priority(runner._hooks[3].priority), get_priority("VERY_LOW"))
    ...
    def test_custom_loop(self):
        ...
        class CustomTrainLoop2(IterBasedTrainLoop):
            ...
            def __init__(self, runner, dataloader, max_iters, warmup_loader, max_warmup_iters):
                super().__init__(runner=runner, dataloader=dataloader, max_iters=max_iters)
                self.warmup_loader = self.runner.build_dataloader(warmup_loader)
            ...
            def warmup_iter(self, data_batch):
                self.runner.call_hook("before_warmup_iter", data_batch=data_batch)
                train_logs = self.runner.model.train_step(data_batch, self.runner.optim_wrapper)
                self.runner.message_hub.update_info("train_logs", train_logs)
                self.runner.call_hook("after_warmup_iter", data_batch=data_batch)
        ...
        self.iter_based_cfg.custom_hooks = [dict(type="TestWarmupHook", priority=50)]
        ...
        for before, after in zip(before_warmup_iter_results, after_warmup_iter_results, strict=False):
    ...
    def test_checkpoint(self):
        ...
        self.assertEqual(ckpt["meta"]["experiment_name"], runner.experiment_name)
        ...
        cfg.optim_wrapper = dict(type="OptimWrapper", optimizer=dict(type="SGD", lr=0.2))
        cfg.param_scheduler = dict(type="MultiStepLR", milestones=[1, 2, 3])
        runner = Runner.from_cfg(cfg)
        runner.resume(path)
        ...
        self.assertEqual(MessageHub.get_current_instance().get_info("epoch"), 2)
        self.assertEqual(MessageHub.get_current_instance().get_info("iter"), 11)
        ...
        path = osp.join(self.temp_dir, "epoch_3.pth")
        self.assertTrue(osp.exists(path))
        self.assertEqual(runner.optim_wrapper["linear1"].param_groups[0]["lr"], 0.0001)
        self.assertIsInstance(runner.optim_wrapper["linear2"].optimizer, Adam)
        self.assertEqual(runner.optim_wrapper["linear2"].param_groups[0]["lr"], 0.0002)

        cfg = copy.deepcopy(self.epoch_based_cfg)
        cfg.experiment_name = "test_checkpoint7"
        cfg.optim_wrapper = dict(
            linear1=dict(type="OptimWrapper", optimizer=dict(type="SGD", lr=0.2)),
            linear2=dict(type="OptimWrapper", optimizer=dict(type="Adam", lr=0.03)),
            constructor="ToyMultipleOptimizerConstructor",
        )
        cfg.model = dict(type="ToyGANModel")
        cfg.param_scheduler = dict(type="MultiStepLR", milestones=[1, 2, 3])
        ...
        self.assertIsInstance(runner.param_schedulers["linear1"][0], MultiStepLR)
        self.assertEqual(runner.param_schedulers["linear1"][0].milestones, {1: 1, 2: 1})
        self.assertEqual(len(runner.param_schedulers["linear2"]), 1)
        self.assertIsInstance(runner.param_schedulers["linear2"][0], MultiStepLR)
        self.assertEqual(runner.param_schedulers["linear2"][0].milestones, {1: 1, 2: 1})
        ...
        assert len(MessageHub.get_current_instance().log_scalars["train/lr"].data[1]) == 3
        ...
        cfg.optim_wrapper = optim_cfg
        cfg.param_scheduler = dict(
            linear1=dict(type="MultiStepLR", milestones=[1, 2, 3]), linear2=dict(type="StepLR", gamma=0.1, step_size=3)
        )
        cfg.model = dict(type="ToyGANModel")
        resumed_cfg = copy.deepcopy(cfg)
        runner = Runner.from_cfg(cfg)
