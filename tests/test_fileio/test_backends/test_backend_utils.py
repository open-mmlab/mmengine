...
from mmengine.fileio.backends import BaseStorageBackend, backends, prefix_to_backends, register_backend
...
def test_register_backend():
    ...
    with pytest.raises(TypeError, match="not a subclass of BaseStorageBackend"):
    ...
    register_backend("example4", ExampleBackend3, prefixes=["prefix2", "prefix3"])
    ...
    register_backend("example6", ExampleBackend4, prefixes="prefix2", force=True)
