...
class TestLocalBackend(TestCase):
    ...
    def test_join_path(self, path_type):
        backend = LocalBackend()
        filepath = backend.join_path(path_type(self.test_data_dir), path_type("file"))
        expected = osp.join(path_type(self.test_data_dir), path_type("file"))
        self.assertEqual(filepath, expected)

        filepath = backend.join_path(path_type(self.test_data_dir), path_type("dir"), path_type("file"))
        expected = osp.join(path_type(self.test_data_dir), path_type("dir"), path_type("file"))
        self.assertEqual(filepath, expected)
    ...
    def test_copyfile(self, path_type):
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            self.assertEqual(backend.copyfile(path_type(src), path_type(dst)), path_type(dst))
            ...
            dst.mkdir()
            self.assertEqual(
                backend.copyfile(path_type(src), path_type(dst)), backend.join_path(path_type(dst), "test.txt")
            )
            self.assertEqual(backend.get_text(backend.join_path(dst, "test.txt")), "disk")
    ...
    def test_copytree(self, path_type):
        ...
        with build_temporary_directory() as tmp_dir:
            ...
            self.assertEqual(backend.copytree(path_type(src), path_type(dst)), path_type(dst))
            ...
            with self.assertRaises(FileExistsError):
                backend.copytree(path_type(src), path_type(Path(tmp_dir) / "dir2"))
    ...
    def test_list_dir_or_file(self, path_type):
        ...
        with build_temporary_directory() as tmp_dir:
            ...
            self.assertEqual(
                set(backend.list_dir_or_file(path_type(tmp_dir))), {"dir1", "dir2", "text1.txt", "text2.txt"}
            )

            # list directories and files recursively
            self.assertEqual(
                set(backend.list_dir_or_file(path_type(tmp_dir), recursive=True)),
                {
                    "dir1",
            ...
            self.assertEqual(set(backend.list_dir_or_file(path_type(tmp_dir), list_file=False)), {"dir1", "dir2"})

            with self.assertRaisesRegex(TypeError, "`suffix` should be None when `list_dir` is True"):
                backend.list_dir_or_file(path_type(tmp_dir), list_file=False, suffix=".txt")

            # only list directories recursively
            self.assertEqual(
                set(backend.list_dir_or_file(path_type(tmp_dir), list_file=False, recursive=True)),
                {"dir1", "dir2", osp.join("dir2", "dir3")},
            )

            # only list files
            self.assertEqual(
                set(backend.list_dir_or_file(path_type(tmp_dir), list_dir=False)), {"text1.txt", "text2.txt"}
            )

            # only list files recursively
            self.assertEqual(
                set(backend.list_dir_or_file(path_type(tmp_dir), list_dir=False, recursive=True)),
                {
                    osp.join("dir1", "text3.txt"),
                    osp.join("dir2", "dir3", "text4.txt"),
                    osp.join("dir2", "img.jpg"),
            ...
            self.assertEqual(
                set(backend.list_dir_or_file(path_type(tmp_dir), list_dir=False, suffix=".txt")),
                {"text1.txt", "text2.txt"},
            )
            self.assertEqual(
                set(backend.list_dir_or_file(path_type(tmp_dir), list_dir=False, suffix=(".txt", ".jpg"))),
                {"text1.txt", "text2.txt"},
            )

            with self.assertRaisesRegex(TypeError, "`suffix` must be a string or tuple of strings"):
                backend.list_dir_or_file(path_type(tmp_dir), list_dir=False, suffix=[".txt", ".jpg"])

            # only list files ending with suffix recursively
            self.assertEqual(
                set(backend.list_dir_or_file(path_type(tmp_dir), list_dir=False, suffix=".txt", recursive=True)),
                {osp.join("dir1", "text3.txt"), osp.join("dir2", "dir3", "text4.txt"), "text1.txt", "text2.txt"},
            )
            ...
            self.assertEqual(
                set(
                    backend.list_dir_or_file(
                        path_type(tmp_dir), list_dir=False, suffix=(".txt", ".jpg"), recursive=True
                    )
                ),
                {
                    osp.join("dir1", "text3.txt"),
