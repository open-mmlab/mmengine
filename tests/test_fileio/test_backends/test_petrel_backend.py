...
try:
    ...
    class MockPetrelClient:
        def __init__(self, enable_mc=True, enable_multi_cluster=False, conf_path=None):
    ...
    class TestPetrelBackend(TestCase):
        ...
        def test_format_path(self):
            backend = PetrelBackend()
            formatted_filepath = backend._format_path("petrel://user\\data\\test.jpg")
            self.assertEqual(formatted_filepath, self.petrel_path)

        def test_replace_prefix(self):
            backend = PetrelBackend()
            self.assertEqual(backend._replace_prefix(self.petrel_path), self.expected_path)

        def test_join_path(self):
            backend = PetrelBackend()
            self.assertEqual(backend.join_path(self.petrel_dir, "file"), f"{self.petrel_dir}/file")
            self.assertEqual(backend.join_path(f"{self.petrel_dir}/", "file"), f"{self.petrel_dir}/file")
            self.assertEqual(backend.join_path(f"{self.petrel_dir}/", "/file"), f"{self.petrel_dir}/file")
            self.assertEqual(backend.join_path(self.petrel_dir, "dir", "file"), f"{self.petrel_dir}/dir/file")

        def test_get(self):
            backend = PetrelBackend()
            with patch.object(backend._client, "Get", return_value=b"petrel") as patched_get:
        ...
        def test_put(self):
            ...
            with patch.object(backend._client, "put") as patched_put:
                ...
                patched_put.assert_called_once_with(self.expected_path, b"petrel")
        ...
        def test_exists(self):
            ...
            # implemented
            with (
                delete_and_reset_method(backend._client, "contains"),
            ...
            with patch.object(backend._client, "contains", return_value=True) as patched_contains:
        ...
        def test_isdir(self):
            ...
            with patch.object(backend._client, "isdir", return_value=True) as patched_contains:
        ...
        def test_get_local_path(self):
            backend = PetrelBackend()
            with (
                patch.object(backend._client, "Get", return_value=b"petrel") as patched_get,
                patch.object(backend._client, "contains", return_value=True) as patch_contains,
            ):
        ...
        def test_copyfile(self):
            ...
            with (
                ...
                patch.object(backend._client, "isdir", return_value=False) as patched_isdir,
            ):
            ...
            with (
                patch.object(backend._client, "Get", return_value=b"petrel") as patched_get,
                patch.object(backend._client, "put") as patched_put,
                patch.object(backend._client, "isdir", return_value=True) as patched_isdir,
            ):
                # dst is a directory
                ...
                patched_get.assert_called_once_with(self.expected_path)
                patched_put.assert_called_once_with(expected_dst, b"petrel")
                patched_isdir.assert_called_once_with(f"{self.expected_dir}/dir")
        ...
        def test_copytree(self):
            ...
            with (
                build_temporary_directory() as tmp_dir,
        ...
        def test_copyfile_from_local(self):
            ...
            with (
                ...
                patched_put.assert_called_once_with(expected_dst, src.open("rb").read())
                patched_isdir.assert_called_once_with(expected_dst)
            ...
            with (
                ...
                self.assertEqual(backend.copyfile_from_local(src, dst), f"{dst}/color.jpg")
                patched_put.assert_called_once_with(expected_dst, src.open("rb").read())
                patched_isdir.assert_called_once_with(f"{self.expected_dir}/dir")
        ...
        def test_copytree_from_local(self):
            ...
            with (
                build_temporary_directory() as tmp_dir,
                patch.object(backend, "copyfile_from_local", side_effect=copyfile_from_local),
                patch.object(backend, "exists", return_value=False),
            ):
        ...
        def test_copyfile_to_local(self):
            ...
            with (
                ...
                dst.mkdir()
                self.assertEqual(backend.copyfile_to_local(src, dst), dst / "test.jpg")
                patched_get.assert_called_once_with(self.expected_path)
                self.assertEqual((dst / "test.jpg").open("rb").read(), b"petrel")
        ...
        def test_copytree_to_local(self):
            ...
            with build_temporary_directory() as tmp_dir, patch.object(backend, "get", side_effect=get):
                dst = f"{tmp_dir}/dir"
        ...
        def test_remove(self):
            ...
            with (
                patch.object(backend._client, "delete") as patched_delete,
                patch.object(backend._client, "isdir", return_value=False) as patched_isdir,
                patch.object(backend._client, "contains", return_value=True) as patched_contains,
            ):
                backend.remove(self.petrel_path)
        ...
        def test_rmtree(self):
            ...
            with build_temporary_directory() as tmp_dir, patch.object(backend, "remove", side_effect=remove):
        ...
        def test_copy_if_symlink_fails(self):
            ...
            with (
                patch.object(backend, "copyfile", side_effect=copyfile),
            ...
            with (
                patch.object(backend, "copytree", side_effect=copytree),
        ...
        def test_list_dir_or_file(self):
            ...
            with build_temporary_directory() as tmp_dir:
                ...
                self.assertEqual(set(backend.list_dir_or_file(tmp_dir)), {"dir1", "dir2", "text1.txt", "text2.txt"})
                ...
                self.assertEqual(set(backend.list_dir_or_file(tmp_dir, list_file=False)), {"dir1", "dir2"})
                with self.assertRaisesRegex(TypeError, "`list_dir` should be False when `suffix` is not None"):
                    backend.list_dir_or_file(tmp_dir, list_file=False, suffix=".txt")

                # only list directories recursively
                self.assertEqual(
                    set(backend.list_dir_or_file(tmp_dir, list_file=False, recursive=True)),
                    {"dir1", "dir2", "/".join(("dir2", "dir3"))},
                )

                # only list files
                self.assertEqual(set(backend.list_dir_or_file(tmp_dir, list_dir=False)), {"text1.txt", "text2.txt"})

                # only list files recursively
                self.assertEqual(
                    set(backend.list_dir_or_file(tmp_dir, list_dir=False, recursive=True)),
                    {
                        "/".join(("dir1", "text3.txt")),
                        "/".join(("dir2", "dir3", "text4.txt")),
                        "/".join(("dir2", "img.jpg")),
                ...
                self.assertEqual(
                    set(backend.list_dir_or_file(tmp_dir, list_dir=False, suffix=".txt")), {"text1.txt", "text2.txt"}
                )
                self.assertEqual(
                    set(backend.list_dir_or_file(tmp_dir, list_dir=False, suffix=(".txt", ".jpg"))),
                    {"text1.txt", "text2.txt"},
                )
                with self.assertRaisesRegex(TypeError, "`suffix` must be a string or tuple of strings"):
                    backend.list_dir_or_file(tmp_dir, list_dir=False, suffix=[".txt", ".jpg"])

                # only list files ending with suffix recursively
                self.assertEqual(
                    set(backend.list_dir_or_file(tmp_dir, list_dir=False, suffix=".txt", recursive=True)),
                    {
                        "/".join(("dir1", "text3.txt")),
                        "/".join(("dir2", "dir3", "text4.txt")),
                        "text1.txt",
                ...
                # only list files ending with suffix
                self.assertEqual(
                    set(backend.list_dir_or_file(tmp_dir, list_dir=False, suffix=(".txt", ".jpg"), recursive=True)),
                    {
                        "/".join(("dir1", "text3.txt")),
                        "/".join(("dir2", "dir3", "text4.txt")),
                        "/".join(("dir2", "img.jpg")),
    ...
    class TestPetrelBackend(TestCase):  # type: ignore
        ...
        def test_copytree(self):
            ...
            self.assertEqual(list(backend.list_dir_or_file(src)), list(backend.list_dir_or_file(dst)))
        ...
        def test_copyfile_from_local(self):
            ...
            self.assertEqual(backend.copyfile_from_local(src, dst), expected_dst)
            self.assertTrue(backend.isfile(expected_dst))
        ...
        def test_copytree_from_local(self):
            ...
            with build_temporary_directory() as tmp_dir:
                backend.copytree_from_local(tmp_dir, self.petrel_dir)
                files = backend.list_dir_or_file(self.petrel_dir, recursive=True)
        ...
        def test_copyfile_to_local(self):
            ...
            with tempfile.TemporaryDirectory() as tmp_dir:
                ...
                dst = Path(tmp_dir) / "dir"
                dst.mkdir()
                self.assertEqual(backend.copyfile_to_local(src, dst), dst / "img.jpg")
        ...
        def test_list_dir_or_file(self):
            ...
            # list directories and files
            self.assertEqual(set(backend.list_dir_or_file(self.petrel_dir)), {"dir1", "dir2", "text1.txt", "text2.txt"})
            ...
            # only list directories
            self.assertEqual(set(backend.list_dir_or_file(self.petrel_dir, list_file=False)), {"dir1", "dir2"})
            with self.assertRaisesRegex(TypeError, "`list_dir` should be False when `suffix` is not None"):
                backend.list_dir_or_file(self.petrel_dir, list_file=False, suffix=".txt")

            # only list directories recursively
            self.assertEqual(
                set(backend.list_dir_or_file(self.petrel_dir, list_file=False, recursive=True)),
                {"dir1", "dir2", "/".join(("dir2", "dir3"))},
            )

            # only list files
            self.assertEqual(set(backend.list_dir_or_file(self.petrel_dir, list_dir=False)), {"text1.txt", "text2.txt"})

            # only list files recursively
            self.assertEqual(
                set(backend.list_dir_or_file(self.petrel_dir, list_dir=False, recursive=True)),
                {
                    "/".join(("dir1", "text3.txt")),
                    "/".join(("dir2", "dir3", "text4.txt")),
                    "/".join(("dir2", "img.jpg")),
            ...
            # only list files ending with suffix
            self.assertEqual(
                set(backend.list_dir_or_file(self.petrel_dir, list_dir=False, suffix=".txt")),
                {"text1.txt", "text2.txt"},
            )
            self.assertEqual(
                set(backend.list_dir_or_file(self.petrel_dir, list_dir=False, suffix=(".txt", ".jpg"))),
                {"text1.txt", "text2.txt"},
            )
            with self.assertRaisesRegex(TypeError, "`suffix` must be a string or tuple of strings"):
                backend.list_dir_or_file(self.petrel_dir, list_dir=False, suffix=[".txt", ".jpg"])

            # only list files ending with suffix recursively
            self.assertEqual(
                set(backend.list_dir_or_file(self.petrel_dir, list_dir=False, suffix=".txt", recursive=True)),
                {"/".join(("dir1", "text3.txt")), "/".join(("dir2", "dir3", "text4.txt")), "text1.txt", "text2.txt"},
            )

            # only list files ending with suffix
            self.assertEqual(
                set(backend.list_dir_or_file(self.petrel_dir, list_dir=False, suffix=(".txt", ".jpg"), recursive=True)),
                {
                    "/".join(("dir1", "text3.txt")),
                    "/".join(("dir2", "dir3", "text4.txt")),
                    "/".join(("dir2", "img.jpg")),
