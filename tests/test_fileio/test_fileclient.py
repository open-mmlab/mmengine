...
class TestFileClient:
    ...
    def test_disk_backend(self):
        ...
        with build_temporary_directory() as tmp_dir:
            ...
            assert set(disk_backend.list_dir_or_file(tmp_dir)) == {"dir1", "dir2", "text1.txt", "text2.txt"}
            # 2. list directories and files recursively
            assert set(disk_backend.list_dir_or_file(tmp_dir, recursive=True)) == {
                "dir1",
            ...
            assert set(disk_backend.list_dir_or_file(tmp_dir, list_file=False)) == {"dir1", "dir2"}
            ...
            assert set(disk_backend.list_dir_or_file(tmp_dir, list_file=False, recursive=True)) == {
                "dir1",
                "dir2",
                osp.join("dir2", "dir3"),
            }
            # 5. only list files
            assert set(disk_backend.list_dir_or_file(tmp_dir, list_dir=False)) == {"text1.txt", "text2.txt"}
            # 6. only list files recursively
            assert set(disk_backend.list_dir_or_file(tmp_dir, list_dir=False, recursive=True)) == {
                osp.join("dir1", "text3.txt"),
                osp.join("dir2", "dir3", "text4.txt"),
                osp.join("dir2", "img.jpg"),
                "text1.txt",
            ...
            assert set(disk_backend.list_dir_or_file(tmp_dir, list_dir=False, suffix=".txt")) == {
                "text1.txt",
                "text2.txt",
            }
            assert set(disk_backend.list_dir_or_file(tmp_dir, list_dir=False, suffix=(".txt", ".jpg"))) == {
                "text1.txt",
                "text2.txt",
            }
            ...
            assert set(disk_backend.list_dir_or_file(tmp_dir, list_dir=False, suffix=".txt", recursive=True)) == {
                osp.join("dir1", "text3.txt"),
                osp.join("dir2", "dir3", "text4.txt"),
                "text1.txt",
                "text2.txt",
            ...
            # 7. only list files ending with suffix
            assert set(
                disk_backend.list_dir_or_file(tmp_dir, list_dir=False, suffix=(".txt", ".jpg"), recursive=True)
            ) == {
                osp.join("dir1", "text3.txt"),
                osp.join("dir2", "dir3", "text4.txt"),
                osp.join("dir2", "img.jpg"),
    ...
    def test_petrel_backend(self, backend, prefix):
        ...
        assert petrel_backend.client._map_path(str(self.img_path)) == str(self.img_path).replace(
            str(self.test_data_dir), petrel_dir
        )
        ...
        with (
            patch.object(petrel_backend.client._client, "delete") as mock_delete,
            patch.object(petrel_backend.client._client, "isdir", return_value=False) as mock_isdir,
            patch.object(petrel_backend.client._client, "contains", return_value=True) as mock_contains,
        ):
            petrel_backend.remove(petrel_path)
        ...
        # raise Exception if `delete` is not implemented
        with (
            delete_and_reset_method(petrel_backend.client._client, "contains"),
            delete_and_reset_method(petrel_backend.client._client, "isdir"),
        ):
        ...
        # test `get_local_path`
        with (
            patch.object(petrel_backend.client._client, "Get", return_value=b"petrel") as mock_get,
            patch.object(petrel_backend.client._client, "contains", return_value=True) as mock_contains,
        ):
        ...
        with build_temporary_directory() as tmp_dir:
            # 1. list directories and files
            assert set(petrel_backend.list_dir_or_file(tmp_dir)) == {"dir1", "dir2", "text1.txt", "text2.txt"}
            # 2. list directories and files recursively
            assert set(petrel_backend.list_dir_or_file(tmp_dir, recursive=True)) == {
                "dir1",
                "/".join(("dir1", "text3.txt")),
                "dir2",
                "/".join(("dir2", "dir3")),
            ...
            # 3. only list directories
            assert set(petrel_backend.list_dir_or_file(tmp_dir, list_file=False)) == {"dir1", "dir2"}
            ...
            # 4. only list directories recursively
            assert set(petrel_backend.list_dir_or_file(tmp_dir, list_file=False, recursive=True)) == {
                "dir1",
                "dir2",
                "/".join(("dir2", "dir3")),
            }
            # 5. only list files
            assert set(petrel_backend.list_dir_or_file(tmp_dir, list_dir=False)) == {"text1.txt", "text2.txt"}
            # 6. only list files recursively
            assert set(petrel_backend.list_dir_or_file(tmp_dir, list_dir=False, recursive=True)) == {
                "/".join(("dir1", "text3.txt")),
                "/".join(("dir2", "dir3", "text4.txt")),
                "/".join(("dir2", "img.jpg")),
                "text1.txt",
            ...
            # 7. only list files ending with suffix
            assert set(petrel_backend.list_dir_or_file(tmp_dir, list_dir=False, suffix=".txt")) == {
                "text1.txt",
                "text2.txt",
            }
            assert set(petrel_backend.list_dir_or_file(tmp_dir, list_dir=False, suffix=(".txt", ".jpg"))) == {
                "text1.txt",
                "text2.txt",
            }
            with pytest.raises(TypeError, match="`suffix` must be a string or tuple of strings"):
            ...
            # 8. only list files ending with suffix recursively
            assert set(petrel_backend.list_dir_or_file(tmp_dir, list_dir=False, suffix=".txt", recursive=True)) == {
                "/".join(("dir1", "text3.txt")),
                "/".join(("dir2", "dir3", "text4.txt")),
                "text1.txt",
                "text2.txt",
            ...
            # 7. only list files ending with suffix
            assert set(
                petrel_backend.list_dir_or_file(tmp_dir, list_dir=False, suffix=(".txt", ".jpg"), recursive=True)
            ) == {
                "/".join(("dir1", "text3.txt")),
                "/".join(("dir2", "dir3", "text4.txt")),
                "/".join(("dir2", "img.jpg")),
    ...
    def test_register_backend(self):
        ...
        FileClient.register_backend(
            "example5", Example7Backend, force=True, prefixes=["example5_prefix1", "example5_prefix2"]
        )
