...
def test_list_from_file():
    ...
    with patch.object(
        ...
        assert filelist == ['1.jpg', '2.jpg', '3.jpg']
        filelist = list_from_file(filename, file_client_args={'prefix': 's3'})
        assert filelist == ['1.jpg', '2.jpg', '3.jpg']
        filelist = list_from_file(filename)
        assert filelist == ['1.jpg', '2.jpg', '3.jpg']
...
def test_dict_from_file():
    ...
    with patch.object(
        ...
        filename = 'http://path/of/your/file'
        mapping = dict_from_file(
            filename, file_client_args={'backend': 'http'})
        assert mapping == {'1': 'cat', '2': ['dog', 'cow'], '3': 'panda'}
        mapping = dict_from_file(filename, file_client_args={'prefix': 'http'})
        assert mapping == {'1': 'cat', '2': ['dog', 'cow'], '3': 'panda'}
        mapping = dict_from_file(filename)
        assert mapping == {'1': 'cat', '2': ['dog', 'cow'], '3': 'panda'}
    ...
    with patch.object(
        ...
        mapping = dict_from_file(
            filename, file_client_args={'backend': 'petrel'})
        assert mapping == {'1': 'cat', '2': ['dog', 'cow'], '3': 'panda'}
        mapping = dict_from_file(filename, file_client_args={'prefix': 's3'})
        assert mapping == {'1': 'cat', '2': ['dog', 'cow'], '3': 'panda'}
        mapping = dict_from_file(filename)
        assert mapping == {'1': 'cat', '2': ['dog', 'cow'], '3': 'panda'}
