...
class TestLRScheduler(TestCase):
    ...
    def test_scheduler_before_optim_warning(self):
        ...
        def call_sch_before_optim_resume():
            scheduler = StepLR(self.optimizer, gamma=0.1, step_size=3, last_step=10)
            scheduler.step()
            self.optimizer.step()
    ...
    def test_effective_interval(self):
        ...
        targets = [single_targets, [x * self.layer2_mult for x in single_targets]]
        scheduler = LinearLR(self.optimizer, start_factor=start_factor, begin=begin, end=begin + iters + 1)
    ...
    def test_step_scheduler(self):
        ...
        targets = [single_targets, [x * self.layer2_mult for x in single_targets]]
        scheduler = StepLR(self.optimizer, gamma=0.1, step_size=3, verbose=True)
        self._test_scheduler_value(scheduler, targets, epochs)
    ...
    def test_multi_step_scheduler(self):
        ...
        targets = [single_targets, [x * self.layer2_mult for x in single_targets]]
        scheduler = MultiStepLR(self.optimizer, gamma=0.1, milestones=[2, 5, 9])
        self._test_scheduler_value(scheduler, targets, epochs)
    ...
    def test_cos_anneal_scheduler(self):
        ...
        # Test default `T_max`
        scheduler = CosineAnnealingLR(self.optimizer, begin=5, end=100, eta_min=eta_min)
    ...
    def test_poly_scheduler(self):
        ...
        scheduler = PolyLR(self.optimizer, power=power, eta_min=min_lr, end=iters + 1)
        self._test_scheduler_value(scheduler, targets, epochs=10)

    def test_cosine_restart_scheduler(self):
        with self.assertRaises(AssertionError):
            CosineRestartLR(self.optimizer, periods=[4, 5], restart_weights=[1, 0.5], eta_min=0, eta_min_ratio=0.1)
        with self.assertRaises(AssertionError):
            CosineRestartLR(self.optimizer, periods=[4, 5], restart_weights=[1, 0.5, 0.0], eta_min=0)
        single_targets = [
            0.05,
            0.0426776,
            0.025,
        ...
        scheduler = CosineRestartLR(self.optimizer, periods=[4, 5], restart_weights=[1, 0.5], eta_min=0)
        self._test_scheduler_value(scheduler, targets, epochs=10)
    ...
    def test_multi_step_scheduler_state_dict(self):
        self._check_scheduler_state_dict(
            lambda: MultiStepLR(self.optimizer, gamma=0.1, milestones=[2, 5, 9]),
            lambda: MultiStepLR(self.optimizer, gamma=0.01, milestones=[1, 4, 6]),
        )

    def test_exp_scheduler_state_dict(self):
        self._check_scheduler_state_dict(
            lambda: ExponentialLR(self.optimizer, gamma=0.1), lambda: ExponentialLR(self.optimizer, gamma=0.01)
        )
    ...
    def test_cosine_scheduler_state_dict(self):
        ...
        eta_min = 1e-10
        self._check_scheduler_state_dict(
            lambda: CosineAnnealingLR(self.optimizer, T_max=epochs, eta_min=eta_min),
            lambda: CosineAnnealingLR(self.optimizer, T_max=epochs // 2, eta_min=eta_min / 2),
            epochs=epochs,
        )
    ...
    def test_cosine_restart_scheduler_state_dict(self):
        self._check_scheduler_state_dict(
            lambda: CosineRestartLR(self.optimizer, periods=[4, 5], restart_weights=[1, 0.5], eta_min=0),
            lambda: CosineRestartLR(self.optimizer, periods=[4, 6], restart_weights=[1, 0.5], eta_min=0),
            epochs=10,
        )
    ...
    def test_step_scheduler_convert_iterbased(self):
        ...
        with self.assertRaises(AssertionError):
            scheduler = StepLR.build_iter_from_epoch(self.optimizer, gamma=0.1, step_size=2, epoch_length=-1)
        ...
        targets = [single_targets, [x * self.layer2_mult for x in single_targets]]
        scheduler = StepLR.build_iter_from_epoch(self.optimizer, gamma=0.1, step_size=2, epoch_length=epoch_length)
        self._test_scheduler_value(scheduler, targets, epochs * epoch_length, param_name="lr")
    ...
    def test_multi_step_scheduler_convert_iterbased(self):
        ...
        scheduler = MultiStepLR.build_iter_from_epoch(
            self.optimizer, gamma=0.1, milestones=[2, 5, 9], epoch_length=epoch_length
        )
    ...
    def test_constant_scheduler_convert_iterbased(self):
        ...
        targets = [single_targets, [x * self.layer2_mult for x in single_targets]]
        scheduler = ConstantLR.build_iter_from_epoch(self.optimizer, factor=1.0 / 2, end=5, epoch_length=epoch_length)
        self._test_scheduler_value(scheduler, targets, epochs * epoch_length)
    ...
    def test_linear_scheduler_convert_iterbased(self):
        ...
        scheduler = LinearLR.build_iter_from_epoch(
            self.optimizer, start_factor=start_factor, end=end, epoch_length=epoch_length
        )
        self._test_scheduler_value(scheduler, targets, epochs)
    ...
    def test_exp_scheduler_convert_iterbased(self):
        ...
        targets = [single_targets, [x * self.layer2_mult for x in single_targets]]
        scheduler = ExponentialLR.build_iter_from_epoch(self.optimizer, gamma=0.9, epoch_length=epoch_length)
        self._test_scheduler_value(scheduler, targets, epochs * epoch_length)
    ...
    def test_cos_anneal_scheduler_convert_iterbased(self):
        ...
        scheduler = CosineAnnealingLR.build_iter_from_epoch(
            self.optimizer, T_max=t, eta_min=eta_min, epoch_length=epoch_length
        )
        self._test_scheduler_value(scheduler, targets, epochs)
    ...
    def test_poly_scheduler_convert_iterbased(self):
        ...
        scheduler = PolyLR.build_iter_from_epoch(
            self.optimizer, power=power, eta_min=min_lr, end=end, epoch_length=epoch_length
        )
        self._test_scheduler_value(scheduler, targets, epochs=10)
    ...
    def test_multi_scheduler_without_overlap_linear_multi_step(self):
        ...
        targets = [single_targets, [x * self.layer2_mult for x in single_targets]]
        scheduler1 = LinearLR(self.optimizer, start_factor=1 / 2, begin=0, end=5)
        scheduler2 = MultiStepLR(self.optimizer, gamma=0.1, milestones=[3, 6], begin=5, end=12)
    ...
    def test_multi_scheduler_without_overlap_exp_cosine(self):
        ...
        targets = [single_targets, [x * self.layer2_mult for x in single_targets]]
        scheduler2 = CosineAnnealingLR(self.optimizer, T_max=5, eta_min=eta_min, begin=5, end=10)
    ...
    def test_multi_scheduler_with_overlap(self):
        ...
        targets = [single_targets, [x * self.layer2_mult for x in single_targets]]
        scheduler1 = LinearLR(self.optimizer, start_factor=1 / 2, begin=0, end=5)
        scheduler2 = MultiStepLR(self.optimizer, gamma=0.1, milestones=[3, 6, 9])
        self._test_scheduler_value([scheduler1, scheduler2], targets, epochs)
    ...
    def test_multi_scheduler_with_gap(self):
        ...
        targets = [single_targets, [x * self.layer2_mult for x in single_targets]]
        scheduler2 = CosineAnnealingLR(self.optimizer, T_max=5, eta_min=eta_min, begin=10, end=15)

        self._test_scheduler_value([scheduler1, scheduler2], targets, epochs)

    def test_onecycle_lr(self):
        # test linear annealing
        target = [1.0, 13.0, 25.0, 21.5, 18.0, 14.5, 11.0, 7.5, 4.0, 0.5]
        scheduler = OneCycleLR(self.optimizer, eta_max=25, final_div_factor=2, total_steps=10, anneal_strategy="linear")
        self._test_scheduler_value(scheduler, [target], 10)
        # test linear annealing three phase
        target = [1.0, 9.0, 17.0, 25.0, 17.0, 9.0, 1.0, 0.75, 0.5, 0.25]
        ...
        scheduler = OneCycleLR(self.optimizer, eta_max=25, final_div_factor=2, total_steps=10)
        self._test_scheduler_value(scheduler, [target], 10)
