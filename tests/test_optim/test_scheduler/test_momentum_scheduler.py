...
class TestMomentumScheduler(TestCase):
    def setUp(self):
        ...
        self.optimizer = optim.SGD(
            [
                ...
                {"params": self.model.conv2.parameters(), "momentum": momentum * self.layer2_mult},
            ],
            lr=0.01,
            momentum=momentum,
            weight_decay=5e-4,
        ...
        self.optimizer_with_betas = optim.Adam(
            [
                {"params": self.model.conv1.parameters()},
                {"params": self.model.conv2.parameters(), "betas": (momentum * self.layer2_mult, 0.999)},
            ],
            lr=0.01,
    ...
    def test_invalid_optimizer(self):
        with self.assertRaisesRegex(ValueError, "optimizer must support momentum when using momentum scheduler"):
    ...
    def test_resume(self):
        ...
        with self.assertRaisesRegex(KeyError, "param 'initial_momentum' is not specified"):
        ...
        for epoch in range(5):
            ...
            if epoch == 4:
                break
            scheduler.step()
        scheduler2 = ExponentialMomentum(self.optimizer, gamma=0.9, last_step=4)
    ...
    def test_scheduler_before_optim_warning(self):
        ...
        def call_sch_before_optim_resume():
            scheduler = StepMomentum(self.optimizer, gamma=0.1, step_size=3, last_step=10)
            scheduler.step()
            self.optimizer.step()
    ...
    def test_effective_interval(self):
        ...
        with self.assertRaisesRegex(ValueError, "end should be larger than begin"):
            StepMomentum(self.optimizer, gamma=0.1, step_size=3, begin=10, end=5)
        ...
        scheduler = LinearMomentum(self.optimizer, start_factor=start_factor, begin=begin, end=begin + iters + 1)
        self._test_scheduler_value(self.optimizer, scheduler, targets, epochs)

    def _test_scheduler_value(self, optimizer, schedulers, targets, epochs=10, param_name="momentum", step_kwargs=None):
        ...
        for epoch in range(epochs):
            for param_group, target in zip(optimizer.param_groups, targets, strict=False):
                assert_allclose(
                    target[epoch],
                ...
                if "betas" in optimizer.defaults:
                    assert_allclose(
                        ...
                        msg="{} is wrong in epoch {}: expected {}, got {}".format(
                            "betas_0", epoch, target[epoch], param_group["betas"][0]
                        ),
                        atol=1e-5,
                        rtol=0,
                    )
    ...
    def test_step_scheduler(self):
        ...
        targets = [single_targets, [x * self.layer2_mult for x in single_targets]]
        scheduler = StepMomentum(self.optimizer, gamma=0.1, step_size=3, verbose=True)
        self._test_scheduler_value(self.optimizer, scheduler, targets, epochs)

        scheduler = StepMomentum(self.optimizer_with_betas, gamma=0.1, step_size=3, verbose=True)
        self._test_scheduler_value(self.optimizer_with_betas, scheduler, targets, epochs)
    ...
    def test_multi_step_scheduler(self):
        ...
        targets = [single_targets, [x * self.layer2_mult for x in single_targets]]
        scheduler = MultiStepMomentum(self.optimizer, gamma=0.1, milestones=[2, 5, 9])
        self._test_scheduler_value(self.optimizer, scheduler, targets, epochs)

        scheduler = MultiStepMomentum(self.optimizer_with_betas, gamma=0.1, milestones=[2, 5, 9])
        self._test_scheduler_value(self.optimizer_with_betas, scheduler, targets, epochs)
    ...
    def test_constant_scheduler(self):
        ...
        scheduler = ConstantMomentum(self.optimizer_with_betas, factor=1.0 / 2, end=5)
        self._test_scheduler_value(self.optimizer_with_betas, scheduler, targets, epochs)
    ...
    def test_linear_scheduler(self):
        ...
        targets = [single_targets, [x * self.layer2_mult for x in single_targets]]
        scheduler = LinearMomentum(self.optimizer, start_factor=start_factor, end=iters + 1)
        self._test_scheduler_value(self.optimizer, scheduler, targets, epochs)

        scheduler = LinearMomentum(self.optimizer_with_betas, start_factor=start_factor, end=iters + 1)
        self._test_scheduler_value(self.optimizer_with_betas, scheduler, targets, epochs)
    ...
    def test_cos_anneal_scheduler(self):
        ...
        t = 10
        eta_min = 1e-10
        single_targets = [eta_min + (0.05 - eta_min) * (1 + math.cos(math.pi * x / t)) / 2 for x in range(epochs)]
        targets = [single_targets, [x * self.layer2_mult for x in single_targets]]
        scheduler = CosineAnnealingMomentum(self.optimizer, T_max=t, eta_min=eta_min)
        self._test_scheduler_value(self.optimizer, scheduler, targets, epochs)

        scheduler = CosineAnnealingMomentum(self.optimizer_with_betas, T_max=t, eta_min=eta_min)
        self._test_scheduler_value(self.optimizer_with_betas, scheduler, targets, epochs)

        # Test default `T_max`
        scheduler = CosineAnnealingMomentum(self.optimizer, begin=5, end=100, eta_min=eta_min)
    ...
    def test_poly_scheduler(self):
        ...
        min_lr = 0.001
        iters = 4
        layer1_targets = [min_lr + (0.05 - min_lr) * (1 - i / iters) ** power for i in range(iters)] + [min_lr] * (
            epochs - iters
        )
        layer2_targets = [
        ...
        scheduler = PolyMomentum(self.optimizer, power=power, eta_min=min_lr, end=iters + 1)
        self._test_scheduler_value(self.optimizer, scheduler, targets, epochs=10)

        scheduler = PolyMomentum(self.optimizer_with_betas, power=power, eta_min=min_lr, end=iters + 1)
        self._test_scheduler_value(self.optimizer_with_betas, scheduler, targets, epochs=10)

    def test_cosine_restart_scheduler(self):
        with self.assertRaises(AssertionError):
            CosineRestartMomentum(
                self.optimizer, periods=[4, 5], restart_weights=[1, 0.5], eta_min=0, eta_min_ratio=0.1
            )
        with self.assertRaises(AssertionError):
            CosineRestartMomentum(self.optimizer, periods=[4, 5], restart_weights=[1, 0.5, 0.0], eta_min=0)
        single_targets = [
            0.05,
            0.0426776,
            0.025,
        ...
        targets = [single_targets, [t * self.layer2_mult for t in single_targets]]
        scheduler = CosineRestartMomentum(self.optimizer, periods=[4, 5], restart_weights=[1, 0.5], eta_min=0)
        self._test_scheduler_value(self.optimizer, scheduler, targets, epochs=10)

        scheduler = CosineRestartMomentum(
            self.optimizer_with_betas, periods=[4, 5], restart_weights=[1, 0.5], eta_min=0
        )
        self._test_scheduler_value(self.optimizer_with_betas, scheduler, targets, epochs=10)
    ...
    def test_reduce_on_plateau_scheduler(self):
        ...
        with self.assertRaises(ValueError):
            ReduceOnPlateauMomentum(self.optimizer, min_value=[0.1, 0.1, 0.1, 0.1])
        with self.assertRaises(ValueError):
        ...
        def _test_value(
            ...
            self._test_scheduler_value(optimizer, scheduler, targets, epochs=epochs, step_kwargs=metrics_list)
    ...
    def test_multi_step_scheduler_state_dict(self):
        self._check_scheduler_state_dict(
            lambda: MultiStepMomentum(self.optimizer, gamma=0.1, milestones=[2, 5, 9]),
            lambda: MultiStepMomentum(self.optimizer, gamma=0.01, milestones=[1, 4, 6]),
        )
    ...
    def test_cosine_scheduler_state_dict(self):
        ...
        eta_min = 1e-10
        self._check_scheduler_state_dict(
            lambda: CosineAnnealingMomentum(self.optimizer, T_max=epochs, eta_min=eta_min),
            lambda: CosineAnnealingMomentum(self.optimizer, T_max=epochs // 2, eta_min=eta_min / 2),
            epochs=epochs,
        )
    ...
    def test_linear_scheduler_state_dict(self):
        ...
        self._check_scheduler_state_dict(
            ...
            lambda: LinearMomentum(self.optimizer, start_factor=0, end_factor=0.3),
            epochs=epochs,
        )
    ...
    def test_cosine_restart_scheduler_state_dict(self):
        self._check_scheduler_state_dict(
            lambda: CosineRestartMomentum(self.optimizer, periods=[4, 5], restart_weights=[1, 0.5], eta_min=0),
            lambda: CosineRestartMomentum(self.optimizer, periods=[4, 6], restart_weights=[1, 0.5], eta_min=0),
            epochs=10,
        )
    ...
    def test_multi_scheduler_without_overlap_linear_multi_step(self):
        ...
        targets = [single_targets, [x * self.layer2_mult for x in single_targets]]
        scheduler1 = LinearMomentum(self.optimizer, start_factor=1 / 2, begin=0, end=5)
        scheduler2 = MultiStepMomentum(self.optimizer, gamma=0.1, milestones=[3, 6], begin=5, end=12)
        self._test_scheduler_value(self.optimizer, [scheduler1, scheduler2], targets, epochs)
    ...
    def test_multi_scheduler_without_overlap_exp_cosine(self):
        ...
        scheduler1 = ExponentialMomentum(self.optimizer, gamma=0.9, begin=0, end=5)
        ...
        targets = [single_targets, [x * self.layer2_mult for x in single_targets]]
        scheduler2 = CosineAnnealingMomentum(self.optimizer, T_max=5, eta_min=eta_min, begin=5, end=10)
    ...
    def test_multi_scheduler_with_overlap(self):
        ...
        targets = [single_targets, [x * self.layer2_mult for x in single_targets]]
        scheduler1 = LinearMomentum(self.optimizer, start_factor=1 / 2, begin=0, end=5)
        scheduler2 = MultiStepMomentum(self.optimizer, gamma=0.1, milestones=[3, 6, 9])
        self._test_scheduler_value(self.optimizer, [scheduler1, scheduler2], targets, epochs)
    ...
    def test_multi_scheduler_with_gap(self):
        ...
        targets = [single_targets, [x * self.layer2_mult for x in single_targets]]
        scheduler2 = CosineAnnealingMomentum(self.optimizer, T_max=5, eta_min=eta_min, begin=10, end=15)
