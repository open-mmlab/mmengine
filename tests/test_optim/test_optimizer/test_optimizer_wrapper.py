...
torch_dtypes = [torch.float16 if dtype is None else getattr(torch, dtype) for dtype in amp_valid_dtypes]
...
class TestOptimWrapper(MultiProcessTestCase):
    ...
    def test_init(self):
        ...
        with self.assertRaisesRegex(AssertionError, "If `clip_grad` is not None"):
    ...
    def test_update_params(self):
        ...
        self.assertEqual(optim_wrapper.scaled_loss, torch.tensor(1.0))
        ...
        loss = torch.tensor(1.0)
        optim_wrapper.update_params(loss)
        self.assertEqual(optim_wrapper.scaled_loss, torch.tensor(1.0) / 3.0)
        optim_wrapper.step.assert_not_called()
        optim_wrapper.zero_grad.assert_not_called()

        # gradient accumulate
        optim_wrapper.update_params(loss)
        self.assertEqual(optim_wrapper._inner_count, 2.0)
        ...
        for _ in range(2):
            ...
            optim_wrapper.step.assert_not_called()
            optim_wrapper.zero_grad.assert_not_called()
        self.assertEqual(optim_wrapper.scaled_loss, torch.tensor(1.0) / 3)
    ...
    def test_ger_lr(self):
        ...
        model = ToyModel()
        optimizer_cfg = dict(type="OptimWrapper", optimizer=dict(type="SGD", lr=0.1))
        paramwise_cfg = dict(custom_keys={"conv1.weight": dict(lr_mult=0.1)})
        optim_constructor = DefaultOptimWrapperConstructor(optimizer_cfg, paramwise_cfg)
        optim_wrapper = optim_constructor(model)
        self.assertEqual(optim_wrapper.get_lr(), dict(base_lr=[0.1], lr=[0.1 * 0.1] + [0.1] * 5))

    def test_get_momentum(self):
        # Get momentum from SGD
        model = ToyModel()
        optim = SGD(model.parameters(), lr=0.0, momentum=0.8)
        optim_wrapper = OptimWrapper(optim)
        self.assertEqual(optim_wrapper.get_momentum(), dict(momentum=[0.8]))
        # Get momentum from Adam
        optim = Adam(model.parameters(), lr=0.0, betas=(0.9, 0.9))
        optim_wrapper = OptimWrapper(optim)
    ...
    def test_zero_grad(self):
        ...
        optimizer.defaults = {}  # adjust this line according to what OptimWrapper expects
    ...
    def test_clip_grads(self):
        ...
        optim_wrapper = OptimWrapper(self.optimizer, clip_grad=dict(max_norm=35))
        ...
        optim_wrapper = OptimWrapper(self.optimizer, clip_grad=dict(type="value", clip_value=0.5))
        loss = self.model(torch.Tensor(1, 1, 1, 1))
        loss.backward()
        optim_wrapper._clip_grad()
    ...
    def test_state_dict(self):
        optim_wrapper = OptimWrapper(self.optimizer)
        self.assertEqual(optim_wrapper.state_dict(), self.optimizer.state_dict())
    ...
    def test_param_groups(self):
        optim_wrapper = OptimWrapper(self.optimizer)
        self.assertEqual(optim_wrapper.param_groups, self.optimizer.param_groups)
...
class TestApexOptimWrapper(TestCase):
    ...
    @unittest.skipIf(
        ...
        reason="`apex` is not available, Please install apex from https://www.github.com/nvidia/apex",
    )
    def test_init(self):
        apex_optim_wrapper = ApexOptimWrapper(optimizer=self.optimizer, opt_level="O1", loss_scale=1)
    ...
    def test_step(self):
        optimizer = MagicMock(spec=Optimizer)
        apex_optim_wrapper = ApexOptimWrapper(optimizer=optimizer, opt_level="O1", loss_scale=1)
        with apex_optim_wrapper.optim_context(self.model):
    ...
    def test_state_dict(self):
        ...
        with apex_optim_wrapper.optim_context(self.model):
            ...
            self.assertDictEqual(optim_state_dict, apex_optim_wrapper.optimizer.state_dict())
    ...
    def test_load_state_dict(self):
        ...
        with apex_optim_wrapper.optim_context(self.model):
            ...
            self.assertDictEqual(optimizer.state_dict(), apex_optim_wrapper.optimizer.state_dict())
            # Test load from optim_wrapper
            apex_optim_wrapper = ApexOptimWrapper(optimizer=self.optimizer)
            apex_optim_wrapper_ = ApexOptimWrapper(optimizer=SGD(self.model.parameters(), lr=0.1))
            apex_optim_wrapper_.load_state_dict(apex_optim_wrapper.state_dict())
            self.assertDictEqual(apex_optim_wrapper.optimizer.state_dict(), apex_optim_wrapper_.optimizer.state_dict())
...
class TestAmpOptimWrapper(TestCase):
    ...
    @unittest.skipIf(
        not torch.cuda.is_available(), reason="`torch.cuda.amp` is only available when pytorch-gpu installed"
    )
    def test_init(self):
        ...
        # Test with dynamic.
        amp_optim_wrapper = AmpOptimWrapper("dynamic", optimizer=self.optimizer)
        self.assertIsNone(amp_optim_wrapper._scale_update_param)
        self.assertIsInstance(amp_optim_wrapper.loss_scaler, GradScaler)

        # Test with dtype float16
        amp_optim_wrapper = AmpOptimWrapper(dtype="float16", optimizer=self.optimizer)
        self.assertIs(amp_optim_wrapper.cast_dtype, torch.float16)

        # Test with dtype bfloat16
        amp_optim_wrapper = AmpOptimWrapper(dtype="bfloat16", optimizer=self.optimizer)
        self.assertIs(amp_optim_wrapper.cast_dtype, torch.bfloat16)

        # Test with dict loss_scale.
        amp_optim_wrapper = AmpOptimWrapper(dict(init_scale=1, growth_factor=2), optimizer=self.optimizer)
        self.assertIsInstance(amp_optim_wrapper.loss_scaler, GradScaler)
        self.assertIsNone(amp_optim_wrapper._scale_update_param)
        with self.assertRaisesRegex(TypeError, "loss_scale must be of type float"):
            AmpOptimWrapper(optimizer=self.optimizer, loss_scale="unknown")

    @parameterized.expand(list(zip(amp_valid_dtypes, strict=False)))
    @unittest.skipIf(
        not torch.cuda.is_available(), reason="`torch.cuda.amp` is only available when pytorch-gpu installed"
    )
    def test_step(self, dtype):
        if dtype is not None and (digit_version(TORCH_VERSION) < digit_version("1.10.0")):
            raise unittest.SkipTest("Require PyTorch version >= 1.10.0 to support `dtype` argument in autocast")
        ...
        amp_optim_wrapper.loss_scaler.step.assert_called_with(amp_optim_wrapper.optimizer)
        amp_optim_wrapper.loss_scaler.update.assert_called_with(amp_optim_wrapper._scale_update_param)
    ...
    def test_state_dict(self):
        ...
        self.assertDictEqual(optim_state_dict, amp_optim_wrapper.optimizer.state_dict())
        self.assertDictEqual(scalar_state_dict, amp_optim_wrapper.loss_scaler.state_dict())
    ...
    def test_load_state_dict(self):
        ...
        self.assertDictEqual(optimizer.state_dict(), amp_optim_wrapper.optimizer.state_dict())
        # Test load from optim_wrapper
        amp_optim_wrapper = AmpOptimWrapper(optimizer=self.optimizer)
        amp_optim_wrapper_ = AmpOptimWrapper(optimizer=SGD(self.model.parameters(), lr=0.1))
        amp_optim_wrapper_.load_state_dict(amp_optim_wrapper.state_dict())
        self.assertDictEqual(amp_optim_wrapper.optimizer.state_dict(), amp_optim_wrapper_.optimizer.state_dict())
        self.assertDictEqual(amp_optim_wrapper.loss_scaler.state_dict(), amp_optim_wrapper_.loss_scaler.state_dict())

    @parameterized.expand(list(zip(amp_valid_dtypes, torch_dtypes, strict=False)))
    @unittest.skipIf(
        not torch.cuda.is_available(), reason="`torch.cuda.amp` is only available when pytorch-gpu installed"
    )
