...
class TestOptimWrapperDict(TestCase):
    def setUp(self) -> None:
        ...
        self.optimizers_wrappers = dict(optim1=self.optim_wrapper1, optim2=self.optim_wrapper2)

    def test_init(self):
        optim_wrapper_dict = OptimWrapperDict(**self.optimizers_wrappers)
        self.assertEqual(optim_wrapper_dict.optim_wrappers, self.optimizers_wrappers)
        with self.assertRaisesRegex(AssertionError, "`OptimWrapperDict` only accept"):
            OptimWrapperDict(**dict(optim1=self.optim1, optim2=self.optim2))

    def test_update_params(self):
        optim_wrapper_dict = OptimWrapperDict(**self.optimizers_wrappers)
        with self.assertRaisesRegex(NotImplementedError, "`update_params` should be called"):
            optim_wrapper_dict.update_params(1)

    def test_backward(self):
        optim_wrapper_dict = OptimWrapperDict(**self.optimizers_wrappers)
        with self.assertRaisesRegex(NotImplementedError, "`backward` should be called"):
            optim_wrapper_dict.backward(1)

    def test_step(self):
        optim_wrapper_dict = OptimWrapperDict(**self.optimizers_wrappers)
        with self.assertRaisesRegex(NotImplementedError, "`step` should be called"):
    ...
    def test_optim_context(self):
        optim_wrapper_dict = OptimWrapperDict(**self.optimizers_wrappers)
        with self.assertRaisesRegex(NotImplementedError, "`optim_context` should be called"):
    ...
    def test_param_groups(self):
        optim_wrapper_dict = OptimWrapperDict(**self.optimizers_wrappers)
        self.assertEqual(optim_wrapper_dict.param_groups["optim1"], self.optim1.param_groups)
        self.assertEqual(optim_wrapper_dict.param_groups["optim2"], self.optim2.param_groups)
    ...
    def test_state_dict(self):
        ...
        self.assertEqual(state_dict["optim1"], self.optim_wrapper1.state_dict())
        self.assertEqual(state_dict["optim2"], self.optim_wrapper2.state_dict())
    ...
    def test_load_state_dict(self):
        ...
        optim_wrapper_dict_load = OptimWrapperDict(optim1=optim_wrapper_load1, optim2=optim_wrapper_load2)
        state_dict = optim_wrapper_dict_save.state_dict()
        optim_wrapper_dict_load.load_state_dict(state_dict)

        self.assertDictEqual(optim_wrapper_dict_load.state_dict(), optim_wrapper_dict_save.state_dict())

    def test_items(self):
        optim_wrapper_dict = OptimWrapperDict(**self.optimizers_wrappers)
        self.assertListEqual(list(optim_wrapper_dict.items()), list(self.optimizers_wrappers.items()))

    def test_values(self):
        optim_wrapper_dict = OptimWrapperDict(**self.optimizers_wrappers)
        self.assertListEqual(list(optim_wrapper_dict.values()), list(self.optimizers_wrappers.values()))

    def test_keys(self):
        optim_wrapper_dict = OptimWrapperDict(**self.optimizers_wrappers)
        self.assertListEqual(list(optim_wrapper_dict.keys()), list(self.optimizers_wrappers.keys()))

    def test_getitem(self):
        optim_wrapper_dict = OptimWrapperDict(**self.optimizers_wrappers)
        self.assertIs(self.optimizers_wrappers["optim1"], optim_wrapper_dict["optim1"])
        self.assertIs(self.optimizers_wrappers["optim2"], optim_wrapper_dict["optim2"])
