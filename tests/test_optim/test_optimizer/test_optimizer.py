...
if not MMCV_FULL_AVAILABLE:
    sys.modules["mmcv.ops"] = MagicMock(DeformConv2d=dict, ModulatedDeformConv2d=dict)
...
class TestBuilder(TestCase):
    ...
    def _check_sgd_optimizer(
        ...
        assert sub_conv1_weight["weight_decay"] == self.base_wd * dwconv_decay_mult
        # sub.conv1.bias
    ...
    def test_dadaptation_optimizers(self):
        ...
        assert set(dadaptation_optimizers).issubset(set(DADAPTATION_OPTIMIZERS))
    ...
    def test_bitsandbytes_optimizers(self):
        ...
        assert set(bitsandbytes_optimizers).issubset(set(BITSANDBYTES_OPTIMIZERS))
    ...
    def test_transformers_optimizers(self):
        ...
        assert set(transformers_optimizers).issubset(set(TRANSFORMERS_OPTIMIZERS))

    def test_build_optimizer(self):
        # test build function without ``constructor`` and ``paramwise_cfg``
        optim_wrapper_cfg = dict(
            type="OptimWrapper",
            optimizer=dict(type="SGD", lr=self.base_lr, weight_decay=self.base_wd, momentum=self.momentum),
        )
        ...
        self.assertRaisesRegex(
            AssertionError,
            "`optim_wrapper_cfg` must contain",
    ...
    def test_build_default_optimizer_constructor(self):
        ...
        optim_constructor_cfg = dict(
            type="DefaultOptimWrapperConstructor", optim_wrapper_cfg=optim_wrapper, paramwise_cfg=paramwise_cfg
        )
        optim_constructor = OPTIM_WRAPPER_CONSTRUCTORS.build(optim_constructor_cfg)
        optim_wrapper = optim_constructor(self.model)
        self._check_sgd_optimizer(optim_wrapper.optimizer, self.model, **paramwise_cfg)
    ...
    def test_build_custom_optimizer_constructor(self):
        ...
        optim_constructor_cfg = dict(
            type="MyOptimizerConstructor", optim_wrapper_cfg=optim_wrapper_cfg, paramwise_cfg=paramwise_cfg
        )
        optim_constructor = OPTIM_WRAPPER_CONSTRUCTORS.build(optim_constructor_cfg)
        ...
        for i, param in enumerate(self.model.parameters()):
            ...
            assert param_group["momentum"] == self.momentum
        # conv1.weight
        assert param_groups[1]["lr"] == self.base_lr * paramwise_cfg["conv1_lr_mult"]
    ...
    def test_default_optimizer_constructor(self):
        ...
        with self.assertRaises(TypeError):
            ...
            optim_wrapper_cfg = dict(type="OptimWrapper", optimizer=dict(lr=0.0001, weight_decay=None))
            paramwise_cfg = ["error"]
            optim_constructor = DefaultOptimWrapperConstructor(optim_wrapper_cfg, paramwise_cfg)
            optim_constructor(self.model)
        ...
        CUSTOM_OPTIMIZERS = Registry("custom optimizer", scope="custom optimizer", parent=OPTIMIZERS)
        ...
        optimizer_cfg = dict(
            optimizer=dict(type="CustomOptimizer", lr=0.1),
        )
    ...
    def test_default_optimizer_constructor_with_model_wrapper(self):
        ...
        self._check_default_optimizer(optim_wrapper.optimizer, model, prefix="module.")
        ...
        optim_constructor = DefaultOptimWrapperConstructor(optim_wrapper_cfg, paramwise_cfg)
        optim_wrapper = optim_constructor(model)
        self._check_sgd_optimizer(optim_wrapper.optimizer, model, prefix="module.", **paramwise_cfg)
    ...
    def test_default_optimizer_constructor_no_grad(self):
        ...
        paramwise_cfg = dict(
            bias_lr_mult=2, bias_decay_mult=0.5, norm_decay_mult=0, dwconv_decay_mult=0.1, dcn_offset_lr_mult=0.1
        )
    ...
    def test_default_optimizer_constructor_bypass_duplicate(self):
        ...
        with self.assertRaisesRegex(ValueError, "some parameters appear in more than one parameter group"):
            optim_constructor = DefaultOptimWrapperConstructor(optim_wrapper_cfg, paramwise_cfg)
        ...
        assert len(optim_wrapper.optimizer.param_groups) == len(model_parameters) == num_params
        self._check_sgd_optimizer(optim_wrapper.optimizer, model, **paramwise_cfg)
        ...
        model_parameters = list(model.parameters())
        num_params = 14 if MMCV_FULL_AVAILABLE else 11
        assert len(optim_wrapper.optimizer.param_groups) == len(model_parameters) - 1 == num_params - 1
    ...
    def test_default_optimizer_constructor_custom_key(self):
        ...
        with self.assertRaises(ValueError):
            ...
            # should be specified
            optim_wrapper_cfg_ = dict(type="OptimWrapper", optimizer=dict(type="SGD", lr=0.01))
            paramwise_cfg_ = dict(custom_keys={".backbone": dict(decay_mult=0.5)})
            optim_constructor = DefaultOptimWrapperConstructor(optim_wrapper_cfg_, paramwise_cfg_)
            optim_constructor(self.model)
        ...
        group_settings.append({"lr": self.base_lr, "momentum": self.momentum, "weight_decay": self.base_wd})

        num_params = 14 if MMCV_FULL_AVAILABLE else 11
        assert len(param_groups) == num_params
        for i, (name, param) in enumerate(self.model.named_parameters()):
            assert torch.equal(param_groups[i]["params"][0], param)
            for group, settings in zip(groups, group_settings, strict=False):
                if name in group:
        ...
        # ExampleModel 2
        optim_wrapper_cfg = dict(
            type="OptimWrapper", optimizer=dict(type="SGD", lr=self.base_lr, momentum=self.momentum)
        )
        ...
        group_settings.append({"lr": self.base_lr, "momentum": self.momentum, "weight_decay": 0})
...
@unittest.skipIf(
    (digit_version(TORCH_VERSION) < digit_version("1.8.0")) or not is_available(),
...
class TestZeroOptimizer(MultiProcessTestCase):
    ...
    def _check_default_optimizer(self, optimizer, model):
        ...
        self.assertEqual(sum(len(param_group["params"]) for param_group in param_groups), len(params_set))
        self.assertTrue(all(param in params_set for param_group in param_groups for param in param_group["params"]))
        state_dict = optimizer.state_dict()
        if get_rank() == 0:
            self.assertEqual(sum(len(pg["params"]) for pg in state_dict["param_groups"]), len(params_set))
        else:
    ...
    def test_zero_redundancy_optimizer(self):
        ...
        with self.assertRaises(TypeError):
            optim_wrapper_cfg = dict(
                optimizer=dict(
                    type="ZeroRedundancyOptimizer", lr=self.base_lr, weight_decay=self.base_wd, momentum=self.momentum
                )
            )
            optim_wrapper = build_optim_wrapper(model, optim_wrapper_cfg)
    ...
    def test_zero_redundancy_optimizer_with_paramwise_cfg(self):
        ...
        # test build function
        paramwise_cfg = dict(
            custom_keys={"conv1": dict(lr_mult=0.0, decay_mult=0.0), "conv2": dict(lr_mult=1.0, decay_mult=2.0)}
        )
        optim_wrapper_cfg = dict(
            optimizer=dict(
                type="ZeroRedundancyOptimizer",
