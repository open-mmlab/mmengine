...
class TestRegistry:
    ...
    def test_register_module(self):
        ...
        with pytest.raises(
            TypeError, match=("name must be None, an instance of str, or a sequence of str, but got <class 'int'>")
        ):
        ...
        with pytest.raises(TypeError, match="force must be a boolean, but got <class 'int'>"):
        ...
        # force=False
        with pytest.raises(KeyError, match="BritishShorthair is already registered in cat at test_registry"):
        ...
        with pytest.raises(TypeError, match="module must be Callable, but got <class 'str'>"):
    ...
    def _build_registry(self):
        ...
        LITTLE_HOUNDS = Registry("little hounds", parent=HOUNDS, scope="little_hound")
        ...
        LITTLE_SAMOYEDS = Registry("little samoyeds", parent=SAMOYEDS, scope="little_samoyed")
    ...
    def test_get(self):
        ...
        assert LITTLE_HOUNDS.get("samoyed.little_samoyed.LittlePedigreeSamoyed") is LittlePedigreeSamoyed

        # get key from its nephews
        assert HOUNDS.get("samoyed.little_samoyed.LittlePedigreeSamoyed") is LittlePedigreeSamoyed
    ...
    def test_build(self, cfg_type):
        ...
        DefaultScope.get_instance(f"test-{time.time()}", scope_name="mid_hound")
        dog = LITTLE_HOUNDS.build(b_cfg)
        assert isinstance(dog, Beagle)

        # `default_scope` can not be found
        DefaultScope.get_instance(f"test2-{time.time()}", scope_name="scope-not-found")
        ...
        s_cfg = cfg_type(dict(_scope_="samoyed", type="MySamoyed", friend=dict(type="hound.BloodHound")))
        ...
        s_cfg = cfg_type(dict(_scope_="samoyed", type="MySamoyed", friend=dict(type="YourSamoyed")))
        dog = DOGS.build(s_cfg)
        assert isinstance(dog, MySamoyed)
        ...
        DOGS.register_module(name="patial dog", module=functools.partial(lambda_dog, name="patial"))
    ...
    def test_switch_scope_and_registry(self):
        ...
        DefaultScope.get_instance(f"scope_{time.time()}", scope_name="chihuahua")
        assert DefaultScope.get_current_instance().scope_name == "chihuahua"

        # Test switch scope and get target registry.
        with CHIHUAHUA.switch_scope_and_registry(scope="hound") as registry:
            assert DefaultScope.get_current_instance().scope_name == "hound"
            assert id(registry) == id(HOUNDS)

        # Test nested-ly switch scope.
        with CHIHUAHUA.switch_scope_and_registry(scope="samoyed") as samoyed_registry:
            assert DefaultScope.get_current_instance().scope_name == "samoyed"
            assert id(samoyed_registry) == id(SAMOYEDS)

            with CHIHUAHUA.switch_scope_and_registry(scope="hound") as hound_registry:
                assert DefaultScope.get_current_instance().scope_name == "hound"
        ...
        with CHIHUAHUA.switch_scope_and_registry(scope="unknown") as registry:
...
def test_build_from_cfg(cfg_type):
    ...
    with pytest.raises(TypeError, match=("cfg should be a dict, ConfigDict or Config, but got <class 'str'>")):
        cfg = "ResNet"
    ...
    with pytest.raises(TypeError, match="type must be a str or valid type, but got <class 'int'>"):
        cfg = dict(type=1000)
        cfg = cfg_type(cfg)
        model = build_from_cfg(cfg, BACKBONES)
    ...
    with pytest.raises(KeyError, match='must contain the key "type"'):
        ...
        model = build_from_cfg(cfg, BACKBONES, default_args=cfg_type(dict(stages=4)))

    # "type" defined using default_args
    cfg = cfg_type(dict(depth=50))
    model = build_from_cfg(cfg, BACKBONES, default_args=cfg_type(dict(type="ResNet")))
    assert isinstance(model, ResNet)
    assert model.depth == 50 and model.stages == 4
    ...
    with pytest.raises(TypeError, match=("registry must be a mmengine.Registry object, but got <class 'str'>")):
        cfg = cfg_type(dict(type="ResNet", depth=50))
        model = build_from_cfg(cfg, "BACKBONES")
