...
class TestUtils(TestCase):
    ...
    def test_count_all_registered_modules(self):
        ...
        self.assertTrue(osp.exists(osp.join(temp_dir.name, "modules_statistic_results.json")))
        registries_info = results["registries"]
        for registry in registries_info:
            self.assertTrue(hasattr(root, registry))
            self.assertEqual(registries_info[registry][0]["num_modules"], len(getattr(root, registry).module_dict))
        temp_dir.cleanup()

        # test not saving results
        count_registered_modules(save_path=None, verbose=False)
        self.assertFalse(osp.exists(osp.join(temp_dir.name, "modules_statistic_results.json")))
