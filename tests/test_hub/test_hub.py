...
# config structure
@pytest.mark.skipif(
    not (is_installed("mmdet") and is_installed("mmpose")), reason="mmdet and mmpose should be installed"
)
def test_get_config():
    # Test load base config.
    base_cfg = get_config("mmdet::_base_/models/faster-rcnn_r50_fpn.py")
    package_path = get_installed_path("mmdet")
    test_base_cfg = Config.fromfile(osp.join(package_path, ".mim", "configs/_base_/models/faster-rcnn_r50_fpn.py"))
    ...
    test_cfg = Config.fromfile(osp.join(package_path, ".mim", "configs/faster_rcnn/faster-rcnn_r50_fpn_1x_coco.py"))
...
@pytest.mark.skipif(not is_installed("mmdet"), reason="mmdet and mmpose should be installed")
def test_get_model():
