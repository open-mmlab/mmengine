...
from mmengine.tune.searchers import Searcher, HYPER_SEARCHERS
...
class TestTuner(RunnerTestCase):
    def setUp(self) -> None:
        super().setUp()
        HYPER_SEARCHERS.register_module(ToySearcher)
        self.hparam_spec = {
    ...
    def test_init(self):
        ...
        self.assertEqual(tuner.epoch_based_cfg, self.epoch_based_cfg)
    ...
    def mock_is_main_process(self, return_value=True):
        return mock.patch('mmengine.dist.is_main_process', return_value=return_value)

    def mock_broadcast(self, side_effect=None):
        return mock.patch('mmengine.dist.broadcast_object_list', side_effect=side_effect)
    ...
    def test_successful_run(self, mock_report_score, mock_train):
        ...
        tuner.searcher.suggest = mock.MagicMock(return_value={'optim_wrapper.optimizer.lr': 0.1})
        ...
        tuner.searcher.record.assert_called_with({'optim_wrapper.optimizer.lr': 0.1}, 0.05)
    ...
    def test_successful_run(self, mock_report_score, mock_train):
        ...
        tuner.searcher.record.assert_called_with({'optim_wrapper.optimizer.lr': 0.1}, float('inf'))
    ...
    def test_tune_method(self, mock_report_score, mock_train):
        ...
        mock_run_trial_return_values = [
            ...
            (mock_hparams[1], mock_scores[1], Exception("Error during training")),
            (mock_hparams[2], mock_scores[2], None),
            (mock_hparams[3], mock_scores[3], None)
        ]
        tuner._run_trial = mock.MagicMock(side_effect=mock_run_trial_return_values)

        with self.mock_is_main_process(), self.mock_broadcast():
            result = tuner.tune()

        self.assertEqual(tuner._history, [(mock_hparams[0], mock_scores[0]), (mock_hparams[2], mock_scores[2]), (mock_hparams[3], mock_scores[3])])
    ...
    def test_with_runner(self):
        ...
        self.assertTrue( set([hparam['optim_wrapper.optimizer.lr'] for hparam, _ in tuner.history]) in self.hparam_spec['optim_wrapper.optimizer.lr']['values'] )
