# Modified from
# https://github.com/facebookresearch/fvcore/blob/main/tests/test_print_model_statistics.py
...
class TestPrintModelStatistics(TestCase):
    ...
    def test_pretty_statistics(self) -> None:
        ...
        self.assertEqual(formatted['a2']['stat1'],
                         '0.123M')  # Others unaffected
    ...
    def test_indicate_uncalled_modules(self) -> None:
        """Tests replacing stats for uncalled modules with an indicator
        string."""
    ...
    def test_remove_zero_statistics(self) -> None:
        ...
        modified_test = {
            mod: stats.copy()
            for mod, stats in test_statistics.items()
        }
        ...
        stats = _remove_zero_statistics(
            modified_test, require_trivial_children=True)

        self.assertTrue('a1.b1.c1.d1'
                        in stats)  # Kept because of non-zero child
    ...
    def test_model_stats_table(self) -> None:
        ...
        table = _model_stats_table(
            string_statistics, stat_columns=stat_columns)
        ...
        self.assertTrue('a2.b1'
                        in table)  # Keep modules with multiple children
        ...
        self.assertTrue('a1.b1.c1.d1'
                        in table)  # Skipping wrappers reaches deeper
    ...
    def test_specific_stats_table(self) -> None:

        model = TestNet()
        inputs = (torch.randn((1, 10)), )

        table = specific_stats_table(FlopCountAnalysis(model, inputs))

        self.assertFalse(' a1 ' in table)  # Wrapper skipping successful
        self.assertTrue('a1.b1.c1.d1.bias' in table)  # this is depth 2
        self.assertTrue('a2.b1.c1.weight' in table)  # Weights included
    ...
    def test_specific_stats_str(self) -> None:
        """Tests calculating model flops and outputting them in model print
        format."""

        model = TestNet()
        inputs = (torch.randn((1, 10)), )
        model_str = specific_stats_str(
        ...
        self.assertTrue('#params: 0.11K, #flops: N/A'
        ...
        model_str = specific_stats_str(
            FlopCountAnalysis(model, inputs).ancestor_mode('caller'),
            activations=ActivationCountAnalysis(
                model, inputs).ancestor_mode('caller'),
        )
    ...
    def test_specific_stats_empty(self) -> None:
        model = nn.ReLU()
        inputs = (torch.randn((1, 10)), )
        table = specific_stats_table(FlopCountAnalysis(model, inputs))
        self.assertGreater(len(table), 0)

        out = specific_stats_str(FlopCountAnalysis(model, inputs))
