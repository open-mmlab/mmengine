...
def test_get_model_complexity_info():
    ...
    complexity_info = get_model_complexity_info(model=model, input_shape=input_shape1)
    flops = FlopAnalyzer(model=model, inputs=(torch.randn(1, *input_shape1),)).total()
    assert complexity_info["flops"] == flops

    # test a network that accepts two tensors as input
    model = NetAcceptTwoTensors()
    complexity_info = get_model_complexity_info(model=model, inputs=(input1, input2))
    flops = FlopAnalyzer(model=model, inputs=(input1, input2)).total()
    params = parameter_count(model=model)[""]
    assert complexity_info["flops"] == flops
    assert complexity_info["params"] == params

    complexity_info = get_model_complexity_info(model=model, input_shape=(input_shape1, input_shape2))
    ...
    scalar = torch.tensor([scalar]) if digit_version(TORCH_VERSION) < digit_version("1.9.0") else scalar
    complexity_info = get_model_complexity_info(model=model, inputs=(input1, scalar))
    ...
    with pytest.raises(ValueError, match="cannot be both set"):
        get_model_complexity_info(model, inputs=input1, input_shape=input_shape1)
