...
from mmengine.analysis.flop_count import (_DEFAULT_SUPPORTED_OPS,
                                          FlopCountAnalysis, flop_count)
...
class ConvNet(nn.Module):
    ...
    def __init__(
        ...
        if transpose:
            ...
            assert (output_padding == 0), 'output_padding is not ' \
                'supported for un-transposed convolutions.'
            kwargs = {}
...
class TestFlopCountAnalysis(TestCase):
    ...
    def test_einsum(self) -> None:
        ...
        self.assertDictEqual(
            flop_dict,
            gt_dict,
            'Einsum operation nct,ncp->ntp failed '
            'to pass the flop count test.',
        )
        ...
        self.assertDictEqual(
            flop_dict,
            gt_dict,
            'Einsum operation ntg,ncg->nct failed '
            'to pass the flop count test.',
        )
...
class TestFlopCountHandles(TestCase):
