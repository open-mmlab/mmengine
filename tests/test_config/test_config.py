...
class TestConfig:
    ...
    def test_init(self, file_format):
        ...
        cfg_file = osp.join(self.data_path, f"config/{file_format}_config/simple_config.{file_format}")
        cfg = Config(cfg_dict, filename=cfg_file)
        assert isinstance(cfg, Config)
        assert cfg.filename == cfg_file
        assert cfg.text == open(cfg_file).read()

        cfg_file = osp.join(self.data_path, f"config/{file_format}_config/test_reserved_key.{file_format}")
    ...
    def test_auto_argparser(self):
        ...
        tmp = sys.argv[1:]
        sys.argv = sys.argv[:2]
        sys.argv[1] = osp.join(self.data_path, "config/py_config/test_merge_from_multiple_bases.py")
    ...
    def test_dump(self, tmp_path):
        ...
        cfg_file = osp.join(self.data_path, "config/py_config/test_dump_pickle_support.py")
        cfg = Config.fromfile(cfg_file)
    ...
    def test_pretty_text(self, tmp_path):
        cfg_file = osp.join(self.data_path, "config/py_config/test_merge_from_multiple_bases.py")
        cfg = Config.fromfile(cfg_file)
        text_cfg_filename = tmp_path / "_text_config.py"
    ...
    def test_repr(self, tmp_path):
        ...
        with open(tmp_txt) as f:
            assert f.read().strip() == f"Config (path: {cfg.filename}): {cfg._cfg_dict.__repr__()}"
    ...
    def test_dict_action(self):
        ...
        args = parser.parse_args(["--options", "item1.a=1", "item2.a=2", "--options", "item2.a=1", "item3=false"])
    ...
    def test_pre_substitute_base_vars(self, tmp_path):
        cfg_path = osp.join(self.data_path, "config", "py_config/test_pre_substitute_base_vars.py")
        ...
        cfg_path = osp.join(self.data_path, "config", "json_config/test_base.json")
        ...
        cfg_path = osp.join(self.data_path, "config", "yaml_config/test_base.yaml")
    ...
    def test_substitute_base_vars(self):
        cfg = dict(item4="_item1.12345", item5=dict(item3="1", item2="_item2_.fswf"), item0=("_item0_.12ed21wq", 1))
        cfg_base = dict(item1=0, item2=[1, 2, 3], item0=(1, 2, 3))
        base_var_dict = {"_item1.12345": "item1", "_item2_.fswf": "item2", "_item0_.12ed21wq": "item0"}
    ...
    @pytest.mark.skipif(
        not is_installed("mmdet") or not is_installed("mmpretrain"), reason="mmdet and mmcls should be installed"
    )
    def test_get_cfg_path_external(self):
        ...
        cfg_name = "mmpretrain::cspnet/cspresnet50_8xb32_in1k.py"
        cfg_path, scope = Config._get_cfg_path(cfg_name, filename)
        assert scope == "mmpretrain"
        osp.isfile(cfg_path)
    ...
    def _predefined_vars(self):
        ...
        cfg_dict_ori = dict(
            item1="{{fileBasename}}", item2="{{ fileDirname}}", item3="abc_{{ fileBasenameNoExtension }}"
        )
        ...
        cfg_file = osp.join(self.data_path, "config/yaml_config/test_predefined_var.yaml")

        # test `use_predefined_variable=False`
        assert Config._file2dict(cfg_file, False)[0]["item1"] == "{{ fileDirname }}"
        assert Config._file2dict(cfg_file)[0]["item1"] == self._get_file_path(osp.dirname(cfg_file))

        # test test_predefined_var.json
        cfg_file = osp.join(self.data_path, "config/json_config/test_predefined_var.json")

        assert Config.fromfile(cfg_file, False)["item1"] == "{{ fileDirname }}"
        assert Config.fromfile(cfg_file)["item1"] == self._get_file_path(osp.dirname(cfg_file))
    ...
    def _merge_from_base(self):
        cfg_file = osp.join(self.data_path, "config/py_config/test_merge_from_base_single.py")
        ...
        with pytest.raises(TypeError):
            Config.fromfile(osp.join(self.data_path, "config/py_config/test_merge_from_base_error.py"))
    ...
    def _merge_from_multiple_bases(self):
        ...
        # Redefine key
        with pytest.raises(KeyError):
            Config.fromfile(osp.join(self.data_path, "config/py_config/test_merge_from_multiple_error.py"))

    def _base_variables(self):
        for file in ["py_config/test_base_variables.py", "json_config/test_base.json", "yaml_config/test_base.yaml"]:
        ...
        for file in [
            ...
            assert cfg_dict["item15"] == dict(
                a=dict(b=dict(a=0)), b=[False], c=["test"], d=[[{"e": 0}], [{"a": 0}, {"b": 1}]], e=[1, 2]
            )

        # test reference assignment for py
        cfg_file = osp.join(self.data_path, "config/py_config/test_pre_substitute_base_vars.py")
        cfg_dict = Config._file2dict(cfg_file)[0]
        ...
        assert cfg.item15 == dict(
            a=dict(b=dict(a=0, b=[5, 6])), b=[False], c=["test"], d=[[{"e": 0}], [{"c": 0}, {"b": 1}]], e=[1, 2]
        )

        # Test use global variable in config function
        cfg_file = osp.join(self.data_path, "config/py_config/test_py_function_global_var.py")
        ...
        with patch("mmengine.config.config.tempfile.TemporaryDirectory", PatchedTempDirectory):
            cfg_file = osp.join(self.data_path, "config/py_config/test_py_modify_key.py")
            cfg = Config._file2dict(cfg_file)[0]
            assert cfg == dict(item1=dict(a=1))

    def _merge_recursive_bases(self):
        cfg_file = osp.join(self.data_path, "config/py_config/test_merge_recursive_bases.py")
        cfg_dict = Config._file2dict(cfg_file)[0]
    ...
    def _merge_intermediate_variable(self):
        cfg_file = osp.join(self.data_path, "config/py_config/test_merge_intermediate_variable_child.py")
        cfg_dict = Config._file2dict(cfg_file)[0]
        # cfg.field
        assert cfg_dict["item1"] == [1, 2]
    ...
    def test_get_external_cfg(self):
        ext_cfg_path = osp.join(self.data_path, "config/py_config/test_get_external_cfg.py")
        ...
        assert ext_cfg._cfg_dict.model._scope_ == "mmdet"
    ...
    def test_build_external_package(self):
        ...
        ext_cfg_path = osp.join(self.data_path, "config/py_config/test_get_external_cfg2.py")
        ext_cfg = Config.fromfile(ext_cfg_path)
        LOCAL_MODELS.build(ext_cfg.model)

        # Test override base variable.
        ext_cfg_path = osp.join(self.data_path, "config/py_config/test_get_external_cfg3.py")
        ext_cfg = Config.fromfile(ext_cfg_path)
    ...
    def test_pickle(self):
        ...
        cfg_path = osp.join(self.data_path, "config/lazy_module_config/toy_model.py")
        cfg = Config.fromfile(cfg_path)
        pickled = pickle.loads(pickle.dumps(cfg))
        assert pickled.__dict__ == cfg.__dict__

    def test_lazy_import(self, tmp_path):
        lazy_import_cfg_path = osp.join(self.data_path, "config/lazy_module_config/toy_model.py")
        cfg = Config.fromfile(lazy_import_cfg_path)
        cfg_dict = cfg.to_dict()
        assert cfg_dict["train_dataloader"]["dataset"]["type"] == "mmengine.testing.runner_test_case.ToyDataset"
        assert cfg_dict["custom_hooks"][0]["type"] == "mmengine.hooks.EMAHook"
        # Dumped config
...
""")  # noqa: E122
        ...
        with pytest.raises(RuntimeError, match="with read_base()"):
            Config.fromfile(osp.join(self.data_path, "config/lazy_module_config/error_mix_using1.py"))

        # Force to import in non-lazy-import mode
        Config.fromfile(osp.join(self.data_path, "config/lazy_module_config/error_mix_using1.py"), lazy_import=False)

        # current lazy-import config, base text config
        with pytest.raises(RuntimeError, match="_base_ ="):
            Config.fromfile(osp.join(self.data_path, "config/lazy_module_config/error_mix_using2.py"))

        cfg = Config.fromfile(osp.join(self.data_path, "config/lazy_module_config/test_mix_builtin.py"))
        ...
        assert set(dumped_cfg.keys()) == {"path", "name", "suffix", "chained", "existed", "cfgname"}
...
class TestConfigDict(TestCase):
    ...
    def test_build_lazy(self):
        ...
        raw = dict(
            a=LazyObject("mmengine"), b=dict(c=2, e=[dict(f=dict(h=LazyObject("mmengine")), g=LazyObject("mmengine"))])
        )
        cfg_dict = ConfigDict(raw)
    ...
    def _check(self, cfg_dict):
        ...
        self._recursive_check_lazy(cfg_dict._to_lazy_dict(), lambda x: x is not mmengine)
        self._recursive_check_lazy(
            cfg_dict._to_lazy_dict(), lambda x: not isinstance(x, ConfigDict) if isinstance(x, dict) else True
        )
        self._recursive_check_lazy(cfg_dict, lambda x: isinstance(x, ConfigDict) if isinstance(x, dict) else True)
