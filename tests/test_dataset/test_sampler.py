...
class TestDefaultSampler(TestCase):
    ...
    def test_dist(self, mock):
        ...
        self.assertEqual(list(sampler), list(range(self.data_length))[2::3] + [1])

        # test round_up=False
        sampler = DefaultSampler(self.dataset, round_up=False, shuffle=False)
        self.assertEqual(sampler.num_samples, np.ceil((self.data_length - 2) / 3))
        self.assertEqual(sampler.total_size, self.data_length)
        self.assertEqual(len(sampler), sampler.num_samples)
    ...
    def test_shuffle(self, mock1, mock2):
        ...
        g = torch.Generator()
        g.manual_seed(10)
        self.assertEqual(list(sampler), torch.randperm(len(self.dataset), generator=g).tolist())
...
class TestInfiniteSampler(TestCase):
    ...
    def test_shuffle(self, mock1, mock2):
        ...
        g = torch.Generator()
        g.manual_seed(42)
        self.assertEqual(samples, torch.randperm(self.data_length, generator=g).tolist())
