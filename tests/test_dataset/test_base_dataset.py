...
from mmengine.dataset import BaseDataset, ClassBalancedDataset, Compose, ConcatDataset, RepeatDataset, force_full_init
...
class TestBaseDataset:
    def setup_method(self):
        self.data_info = dict(filename="test_img.jpg", height=604, width=640, sample_idx=0)
    ...
    def test_init(self):
        ...
        with pytest.raises(TypeError):
            BaseDataset(data_root=osp.join(osp.dirname(__file__), "../data/"), data_prefix=dict(img_path="imgs"))
        ...
        # return `list[dict]`
        BaseDataset.parse_data_info = MagicMock(return_value=[self.data_info, self.data_info.copy()])
        dataset = BaseDataset(
            data_root=osp.join(osp.dirname(__file__), "../data/"),
            data_prefix=dict(img_path="imgs"),
            ann_file="annotations/dummy_annotation.json",
        ...
        with pytest.raises(TypeError):
            BaseDataset.parse_data_info = MagicMock(return_value=[self.data_info, "xxx"])
            BaseDataset(
                data_root=osp.join(osp.dirname(__file__), "../data/"),
                data_prefix=dict(img_path="imgs"),
                ann_file="annotations/dummy_annotation.json",
    ...
    def test_meta(self):
        ...
        assert dataset.metainfo == dict(dataset_type="test_dataset", task_name="test_task", empty_list=[])

        # test dataset.metainfo with setting METAINFO in self.base_dataset
        dataset_type = "new_dataset"
        BaseDataset.METAINFO = dict(dataset_type=dataset_type, classes=("dog", "cat"))
        ...
        assert dataset.metainfo == dict(
            dataset_type=dataset_type, task_name="test_task", classes=("dog", "cat"), empty_list=[]
        )
        ...
        assert BaseDataset.METAINFO == dict(dataset_type=dataset_type, classes=("dog", "cat"))
        assert dataset.metainfo == dict(
            dataset_type=dataset_type, task_name="new_task", classes=("dog",), empty_list=[]
        )
        ...
        assert BaseDataset.METAINFO == dict(dataset_type=dataset_type, classes=("dog", "cat", "fish"))
        assert dataset.metainfo == dict(
            dataset_type=dataset_type, task_name="new_task", classes=("dog",), empty_list=[]
        )
        ...
        # self.base_dataset
        metainfo = dict(classes=osp.join(osp.dirname(__file__), "../data/meta/classes.txt"))
        dataset = BaseDataset(
            data_root=osp.join(osp.dirname(__file__), "../data/"),
            data_prefix=dict(img_path="imgs"),
            ann_file="annotations/dummy_annotation.json",
        ...
        assert dataset.metainfo == dict(
            dataset_type=dataset_type, task_name="test_task", classes=["dog"], empty_list=[]
        )
        ...
        assert dataset.metainfo == dict(dataset_type=dataset_type, classes=("dog",))
        ...
        assert ToyDataset.METAINFO == dict(dataset_type=dataset_type, classes=("bird",))
        assert BaseDataset.METAINFO == dict(dataset_type=dataset_type, classes=("dog", "cat", "fish"))
    ...
    def test_get_data_info(self, lazy_init):
        ...
        assert data_info["img_path"] == osp.join(data_root, "imgs", "test_img.jpg")
    ...
    @pytest.mark.parametrize("lazy_init, serialize_data", ([True, False], [False, True], [True, True], [False, False]))
...
class TestConcatDataset:
    def setup_method(self):
        ...
        # test init
        self.cat_datasets = ConcatDataset(datasets=[self.dataset_a, self.dataset_b])
    ...
    def test_init(self):
        ...
        for i in range(len(cat_datasets)):
            assert cat_datasets.get_data_info(i) == self.cat_datasets.get_data_info(i)
        ...
        with pytest.raises(TypeError):
            ConcatDataset(datasets=[self.dataset_a, dataset_cfg_b], ignore_keys=1)
    ...
    def test_full_init(self):
        ...
        # raise error.
        with pytest.raises(ValueError):
            ConcatDataset(datasets=[self.dataset_a, dataset_b], ignore_keys=["a"])
        # Different meta information with `ignore_keys` will not raise error.
        cat_datasets = ConcatDataset(datasets=[self.dataset_a, dataset_b], ignore_keys="classes")
    ...
    def test_length(self):
        assert len(self.cat_datasets) == (len(self.dataset_a) + len(self.dataset_b))

    def test_getitem(self):
        assert (self.cat_datasets[0]["imgs"] == self.dataset_a[0]["imgs"]).all()
        assert (self.cat_datasets[0]["imgs"] != self.dataset_b[0]["imgs"]).all()

        assert (self.cat_datasets[-1]["imgs"] == self.dataset_b[-1]["imgs"]).all()
        assert (self.cat_datasets[-1]["imgs"] != self.dataset_a[-1]["imgs"]).all()

    def test_get_data_info(self):
        assert self.cat_datasets.get_data_info(0) == self.dataset_a.get_data_info(0)
        assert self.cat_datasets.get_data_info(0) != self.dataset_b.get_data_info(0)

        assert self.cat_datasets.get_data_info(-1) == self.dataset_b.get_data_info(-1)
        assert self.cat_datasets.get_data_info(-1) != self.dataset_a.get_data_info(-1)

    def test_get_ori_dataset_idx(self):
        assert self.cat_datasets._get_ori_dataset_idx(3) == (1, 3 - len(self.dataset_a))
        assert self.cat_datasets._get_ori_dataset_idx(-1) == (1, len(self.dataset_b) - 1)
        with pytest.raises(ValueError):
...
class TestRepeatDataset:
    def setup_method(self):
        ...
        self.repeat_times = 5
        # test init
        self.repeat_datasets = RepeatDataset(dataset=self.dataset, times=self.repeat_times)
    ...
    def test_init(self):
        ...
        repeat_dataset = RepeatDataset(dataset=dataset_cfg, times=self.repeat_times)
        ...
        for i in range(len(repeat_dataset)):
            assert repeat_dataset.get_data_info(i) == self.repeat_datasets.get_data_info(i)
    ...
    def test_full_init(self):
        ...
        assert len(self.repeat_datasets) == self.repeat_times * len(self.dataset)
        self.repeat_datasets.full_init()
    ...
    def test_length(self):
        assert len(self.repeat_datasets) == len(self.dataset) * self.repeat_times

    def test_getitem(self):
        for i in range(self.repeat_times):
            assert self.repeat_datasets[len(self.dataset) * i] == self.dataset[0]

    def test_get_data_info(self):
        for i in range(self.repeat_times):
            assert self.repeat_datasets.get_data_info(len(self.dataset) * i) == self.dataset.get_data_info(0)
...
class TestClassBalancedDataset:
    def setup_method(self):
        ...
        # test init
        self.cls_banlanced_datasets = ClassBalancedDataset(dataset=self.dataset, oversample_thr=1e-3)
    ...
    def test_init(self):
        ...
        cls_banlanced_datasets = ClassBalancedDataset(dataset=dataset_cfg, oversample_thr=1e-3)
        ...
        for i in range(len(cls_banlanced_datasets)):
            assert cls_banlanced_datasets.get_data_info(i) == self.cls_banlanced_datasets.get_data_info(i)
            assert cls_banlanced_datasets[i] == self.cls_banlanced_datasets[i]
    ...
    def test_getitem(self):
        for i in range(len(self.repeat_indices)):
            assert self.cls_banlanced_datasets[i] == self.dataset[self.repeat_indices[i]]

    def test_get_data_info(self):
        for i in range(len(self.repeat_indices)):
            assert self.cls_banlanced_datasets.get_data_info(i) == self.dataset.get_data_info(self.repeat_indices[i])

    def test_get_cat_ids(self):
        for i in range(len(self.repeat_indices)):
            assert self.cls_banlanced_datasets.get_cat_ids(i) == self.dataset.get_cat_ids(self.repeat_indices[i])
