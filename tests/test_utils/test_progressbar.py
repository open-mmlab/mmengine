...
class TestProgressBar:
    def test_start(self):
        ...
        reset_string_io(out)
        prog_bar = mmengine.ProgressBar(bar_width=bar_width, start=False, file=out)
        assert out.getvalue() == ""
        reset_string_io(out)
        prog_bar.start()
        assert out.getvalue() == "completed: 0, elapsed: 0s"
        ...
        reset_string_io(out)
        prog_bar = mmengine.ProgressBar(10, bar_width=bar_width, start=False, file=out)
        assert out.getvalue() == ""
        reset_string_io(out)
        prog_bar.start()
        assert out.getvalue() == f"[{' ' * bar_width}] 0/10, elapsed: 0s, ETA:"

    @skipIf(platform.system() != "Linux", reason="Only test `TestProgressBar.test_update` in Linux")
    def test_update(self):
        ...
        reset_string_io(out)
        prog_bar.update()
        assert out.getvalue() == f"\r[{'>' * 2 + ' ' * 18}] 1/10, 1.0 task/s, elapsed: 1s, ETA:     9s"

    @skipIf(platform.system() != "Linux", reason="Only test `TestProgressBar.test_adaptive_length` in Linux")
...
def test_track_progress():
    ...
    ret = mmengine.track_progress(return_itself, ((i for i in [1, 2, 3]), 3), bar_width=3, file=out)
    assert ret == [1, 2, 3]

    # tasks is a range object
    ret = mmengine.track_progress(return_itself, range(1, 4), bar_width=3, file=out)
    assert ret == [1, 2, 3]
...
def test_track_iter_progress():
    ...
    ret = []
    count = []
    for i, num in enumerate(mmengine.track_iter_progress([1, 2, 3], bar_width=3, file=out)):
        ret.append(num)
        count.append(i)
    assert ret == [1, 2, 3]
    assert count == [0, 1, 2]
...
def test_track_parallel_progress():
    # tasks is a list
    out = StringIO()
    ret = mmengine.track_parallel_progress(return_itself, [1, 2, 3, 4], 2, bar_width=4, file=out)
    assert ret == [1, 2, 3, 4]

    # tasks is an iterable object
    ret = mmengine.track_parallel_progress(return_itself, ((i for i in [1, 2, 3, 4]), 4), 2, bar_width=4, file=out)
    assert ret == [1, 2, 3, 4]

    # tasks is a range object
    ret = mmengine.track_parallel_progress(return_itself, range(1, 5), 2, bar_width=4, file=out)
    assert ret == [1, 2, 3, 4]
