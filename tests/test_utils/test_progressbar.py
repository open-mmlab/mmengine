...
class TestProgressBar:
    ...
    def test_start(self):
        ...
        reset_string_io(out)
        prog_bar = mmengine.ProgressBar(bar_width=bar_width,
                                        start=False,
                                        file=out)
        assert out.getvalue() == ''
        reset_string_io(out)
...
def test_track_progress():
    ...
    ret = mmengine.track_progress(return_itself, ((i for i in [1, 2, 3]), 3),
                                  bar_width=3,
                                  file=out)
    assert ret == [1, 2, 3]

    # tasks is a range object
    ret = mmengine.track_progress(return_itself,
                                  range(1, 4),
                                  bar_width=3,
                                  file=out)
    assert ret == [1, 2, 3]
...
def test_track_parallel_progress():
    # tasks is a list
    out = StringIO()
    ret = mmengine.track_parallel_progress(return_itself, [1, 2, 3, 4],
                                           2,
                                           bar_width=4,
                                           file=out)
    assert ret == [1, 2, 3, 4]
    ...
    # tasks is an iterable object
    ret = mmengine.track_parallel_progress(return_itself,
