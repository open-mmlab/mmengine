...
def test_to_ntuple():
    ...
    assert to_3tuple(single_number) == (single_number, single_number, single_number)
    assert to_4tuple(single_number) == (single_number, single_number, single_number, single_number)
    assert to_ntuple(5)(single_number) == (single_number, single_number, single_number, single_number, single_number)
    assert to_ntuple(6)(single_number) == (
        single_number,
        single_number,
        single_number,
        single_number,
...
def test_import_modules_from_strings():
    ...
    with pytest.warns(UserWarning):
        imported = import_modules_from_strings("_not_implemented_module", allow_failed_imports=True)
        assert imported is None
    with pytest.warns(UserWarning):
        imported = import_modules_from_strings(["os.path", "_not_implemented"], allow_failed_imports=True)
...
def test_deprecated_function():
    ...
    expected_docstring = """.. deprecated:: 0.2.0
...
def test_apply_to():
    ...
    result = apply_to(data, lambda x: isinstance(x, torch.Tensor), lambda x: x.numpy())
    ...
    result = apply_to(
        data, lambda x: isinstance(x, int) or x == "test", lambda x: torch.Tensor(x) if isinstance(x, int) else "train"
    )
...
def test_locate():
    ...
    assert get_object_from_string("mmengine.config.Config") is config_module.Config
    assert get_object_from_string("mmengine.config.Config.fromfile") is config_module.Config.fromfile
