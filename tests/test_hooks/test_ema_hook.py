...
class TestEMAHook(RunnerTestCase):
    ...
    def test_init(self):
        ...
        with self.assertRaisesRegex(AssertionError, "`begin_iter` and `begin_epoch`"):
    ...
    def test_before_train(self):
        cfg = copy.deepcopy(self.epoch_based_cfg)
        cfg.custom_hooks = [dict(type="EMAHook", begin_epoch=cfg.train_cfg.max_epochs - 1)]
        runner = self.build_runner(cfg)
        ema_hook = self._get_ema_hook(runner)
        ...
        cfg.custom_hooks = [dict(type="EMAHook", begin_iter=cfg.train_cfg.max_iters + 1)]
        runner = self.build_runner(cfg)
        ema_hook = self._get_ema_hook(runner)
    ...
    def test_after_train_iter(self):
        ...
        for src, ema in zip(src_model.parameters(), ema_model.parameters(), strict=False):
    ...
    def test_before_save_checkpoint(self):
        ...
        for key in ori_checkpoint["state_dict"].keys():
            assert_allclose(
                ori_checkpoint["state_dict"][key].cpu(), checkpoint["ema_state_dict"][f"module.{key}"].cpu()
            )

            assert_allclose(ema_hook.ema_model.state_dict()[f"module.{key}"].cpu(), checkpoint["state_dict"][key].cpu())
    ...
    def test_after_load_checkpoint(self):
        ...
        for key in checkpoint["state_dict"].keys():
            assert_allclose(checkpoint["state_dict"][key].cpu(), ema_hook.ema_model.state_dict()[f"module.{key}"].cpu())
        ...
        runner._resume = True
        checkpoint = dict(
            state_dict=ToyModel().state_dict(), ema_state_dict=ExponentialMovingAverage(ToyModel()).state_dict()
        )
        ori_checkpoint = copy.deepcopy(checkpoint)
        ema_hook.after_load_checkpoint(runner, checkpoint)
        for key in ori_checkpoint["state_dict"].keys():
            assert_allclose(
                ori_checkpoint["state_dict"][key].cpu(), ema_hook.ema_model.state_dict()[f"module.{key}"].cpu()
            )
    ...
    def test_with_runner(self):
        ...
        torch.save(checkpoint, osp.join(self.temp_dir.name, "without_ema_state_dict.pth"))

        cfg.load_from = osp.join(self.temp_dir.name, "without_ema_state_dict.pth")
        runner = self.build_runner(cfg)
        runner.test()
        ...
        runner = self.build_runner(cfg)
        runner.train()
        state_dict = torch.load(osp.join(self.temp_dir.name, "epoch_4.pth"), map_location="cpu")
        ...
        runner = self.build_runner(cfg)
        runner.train()
        state_dict = torch.load(osp.join(self.temp_dir.name, "iter_4.pth"), map_location="cpu")
        self.assertIn("ema_state_dict", state_dict)
        for k, v in state_dict["state_dict"].items():
            assert_allclose(v, state_dict["ema_state_dict"]["module." + k])
        state_dict = torch.load(osp.join(self.temp_dir.name, "iter_5.pth"), map_location="cpu")
        self.assertIn("ema_state_dict", state_dict)
    ...
    def _test_swap_parameters(self, func_name, *args, **kwargs):
        ...
        for src, ema, swapped_src, swapped_ema in zip(
            src_model.parameters(), ema_model.parameters(), swapped_src.parameters(), swapped_ema.parameters(), strict=False
        ):
