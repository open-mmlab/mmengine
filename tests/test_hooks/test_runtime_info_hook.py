...
class TestRuntimeInfoHook(RunnerTestCase):
    ...
    def test_before_train_iter(self):
        ...
        self.assertEqual(runner.message_hub.get_scalar("train/lr").current(), lr)

        with self.assertRaisesRegex(AssertionError, "runner.optim_wrapper.get_lr()"):
        ...
        optim_wrapper_dict = OptimWrapperDict(key1=optim_wrapper1, key2=optim_wrapper2)
        runner.optim_wrapper = optim_wrapper_dict
        hook.before_train_iter(runner, batch_idx=2, data_batch=None)
        self.assertEqual(runner.message_hub.get_scalar("train/key1.lr").current(), 0.01)
        self.assertEqual(runner.message_hub.get_scalar("train/key2.lr").current(), 0.02)

    def test_after_train_iter(self):
        cfg = copy.deepcopy(self.epoch_based_cfg)
        runner = self.build_runner(cfg)
        hook = self._get_runtime_info_hook(runner)
        hook.after_train_iter(runner, batch_idx=2, data_batch=None, outputs={"loss_cls": 1.111})
        self.assertEqual(runner.message_hub.get_scalar("train/loss_cls").current(), 1.111)
    ...
    def test_after_val_epoch(self):
        ...
        self.assertEqual(runner.message_hub.get_scalar("val/acc").current(), 0.8)
    ...
    def test_after_test_epoch(self):
        ...
        self.assertEqual(runner.message_hub.get_scalar("test/acc").current(), 0.8)
    ...
    def test_scalar_check(self):
        ...
        self.assertEqual(runner.message_hub.get_scalar("val/acc_f32").current(), 5)
        self.assertEqual(runner.message_hub.get_scalar("val/acc_i32").current(), 5)
        self.assertEqual(runner.message_hub.get_scalar("val/acc_u8").current(), 5)
        self.assertEqual(runner.message_hub.get_scalar("val/acc_ndarray").current(), 5)
        ...
        self.assertEqual(runner.message_hub.get_scalar("val/acc_f32").current(), 5)
        self.assertEqual(runner.message_hub.get_scalar("val/acc_i64").current(), 5)
        self.assertEqual(runner.message_hub.get_scalar("val/acc_tensor").current(), 5)
