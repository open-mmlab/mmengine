...
class TestEarlyStoppingHook(RunnerTestCase):
    ...
    def test_after_val_epoch(self):
        ...
        with self.assertRaises(RuntimeError):
            metrics = {"accuracy/top1": 0.5, "loss": 0.23}
            hook = EarlyStoppingHook(monitor="acc", rule="greater", strict=True)
            hook.after_val_epoch(runner, metrics)

        # Check largest value
        runner = get_mock_runner()
        metrics = [{"accuracy/top1": i / 9.0} for i in range(8)]
        hook = EarlyStoppingHook(monitor="accuracy/top1", rule="greater")
        ...
        runner = get_mock_runner()
        metrics = [{"loss": i / 9.0} for i in range(8, 0, -1)]
        hook = EarlyStoppingHook(monitor="loss")
        for metric in metrics:
            hook.after_val_epoch(runner, metric)
            if runner.train_loop.stop_training:
        ...
        hook = EarlyStoppingHook(monitor="accuracy/top1", rule="greater", min_delta=1)
        for metric in metrics:
            hook.after_val_epoch(runner, metric)
            if runner.train_loop.stop_training:
                break
        ...
        runner = get_mock_runner()
        metrics = [{"accuracy/top1": i} for i in torch.linspace(98, 99, 8)]
        hook = EarlyStoppingHook(monitor="accuracy/top1", rule="greater", min_delta=1, patience=10)
        for metric in metrics:
            hook.after_val_epoch(runner, metric)
            if runner.train_loop.stop_training:
                break
        ...
        runner = get_mock_runner()
        metrics = [{"accuracy/top1": i} for i in torch.linspace(98, 99, 8)]
        hook = EarlyStoppingHook(monitor="accuracy/top1", rule="greater", stopping_threshold=98.5, patience=0)
        for metric in metrics:
            hook.after_val_epoch(runner, metric)
            if runner.train_loop.stop_training:
                break
    ...
    def test_with_runner(self):
        ...
        runner = Runner(
            ...
            val_dataloader=dict(
                dataset=DummyDataset(), sampler=dict(type="DefaultSampler", shuffle=False), batch_size=3, num_workers=0
            ),
