...
class TestLoggerHook(RunnerTestCase):
    def test_init(self):
        ...
        with self.assertLogs(logger, level="WARNING"):
            LoggerHook(out_dir=self.temp_dir.name, file_client_args=dict(backend="disk"))

        with self.assertRaisesRegex(ValueError, '"file_client_args" and "backend_args" cannot be '):
            LoggerHook(
                out_dir=self.temp_dir.name, file_client_args=dict(enable_mc=True), backend_args=dict(enable_mc=True)
            )
    ...
    def test_after_train_iter(self):
        ...
        runner = MagicMock()
        runner.log_processor.get_log_after_iter = MagicMock(return_value=(dict(), "log_str"))
    ...
    def test_after_val_epoch(self):
        ...
        runner.log_processor.get_log_after_epoch = MagicMock(
            return_value=({"time": 1, "datatime": 1, "acc": 0.8}, "string")
        )
        ...
        calls = [
            call({"time": 1, "datatime": 1, "acc": 0.8}, **args),
        ]
        self.assertEqual(len(calls), len(runner.visualizer.add_scalars.mock_calls))
        ...
        runner.log_processor.get_log_after_epoch = MagicMock(
            return_value=({"time": 5, "datatime": 5, "acc": 0.5}, "string")
        )
        logger_hook.after_val_epoch(runner)
        ...
        calls = [
            call({"time": 1, "datatime": 1, "acc": 0.8}, **args),
            call({"time": 5, "datatime": 5, "acc": 0.5}, **args),
        ]
        self.assertEqual(len(calls), len(runner.visualizer.add_scalars.mock_calls))
        runner.visualizer.add_scalars.assert_has_calls(calls)
    ...
    def test_after_test_epoch(self):
        ...
        runner.log_processor.get_log_after_epoch = MagicMock(
            return_value=(dict(a=1, b=2, c={"list": [1, 2]}, d=torch.tensor([1, 2, 3])), "log_str")
        )
        ...
        json_content = load(osp.join(runner.log_dir, "test_after_test_epoch.json"))
    ...
    def test_with_runner(self):
        ...
        runner.train()
        json_path = osp.join(runner._log_dir, "vis_data", f"{runner.timestamp}.json")
        ...
        # Test out_suffix
        cfg.default_hooks.logger = dict(type="LoggerHook", out_dir=out_dir, out_suffix=".log")
        runner = self.build_runner(cfg)
        runner.train()
        filenames = scandir(out_dir, recursive=True)
        self.assertTrue(all(filename.endswith(".log") for filename in filenames))

        # Test keep_local=False
        cfg.default_hooks.logger = dict(type="LoggerHook", out_dir=out_dir, keep_local=False)
        runner = self.build_runner(cfg)
        runner.train()
        filenames = scandir(runner._log_dir, recursive=True)

        for filename in filenames:
            self.assertFalse(filename.endswith((".log", ".json", ".py", ".yaml")), f"{filename} should not be kept.")
