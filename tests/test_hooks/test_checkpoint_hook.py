...
class TestCheckpointHook(RunnerTestCase):
    ...
    def test_init(self):
        ...
        with self.assertRaisesRegex(ValueError, '"file_client_args" and "backend_args" cannot be set at the same time'):
            CheckpointHook(file_client_args={"backend": "disk"}, backend_args={"backend": "local"})
        ...
        with self.assertRaisesRegex(AssertionError, 'Number of "rule" must be'):
        ...
        hook = CheckpointHook(interval=2, by_epoch=False, save_best=["acc", "mIoU"])
    ...
    def test_before_train(self):
        ...
        self.assertIs(checkpoint_hook.file_backend, checkpoint_hook.file_client)
        ...
        checkpoint_hook = CheckpointHook(interval=1, by_epoch=True, out_dir="test_dir")
        checkpoint_hook.before_train(runner)
        self.assertEqual(checkpoint_hook.out_dir, osp.join("test_dir", osp.basename(cfg.work_dir)))
        ...
        checkpoint_hook.before_train(runner)
        self.assertEqual(checkpoint_hook.best_ckpt_path_dict, dict(acc=None, mIoU=None))
        ...
        checkpoint_hook.before_train(runner)
        self.assertEqual(checkpoint_hook.best_ckpt_path_dict, dict(acc="best_acc", mIoU=None))
    ...
    def test_after_val_epoch(self):
        ...
        with self.assertLogs(runner.logger, level="WARNING"):
            checkpoint_hook = CheckpointHook(interval=2, by_epoch=True, save_best="auto")
            checkpoint_hook.after_val_epoch(runner, {})

        # if save_best is None,no best_ckpt meta should be stored
        checkpoint_hook = CheckpointHook(interval=2, by_epoch=True, save_best=None)
        checkpoint_hook.before_train(runner)
        checkpoint_hook.after_val_epoch(runner, {})
        ...
        best_ckpt_path = checkpoint_hook.file_client.join_path(checkpoint_hook.out_dir, best_ckpt_name)
        ...
        self.assertEqual(runner.message_hub.get_info("best_ckpt"), best_ckpt_path)

        # # when `save_best` is set to `acc`, it should update greater value
        checkpoint_hook = CheckpointHook(interval=2, by_epoch=True, save_best="acc")
        checkpoint_hook.before_train(runner)
        metrics["acc"] = 0.8
        checkpoint_hook.after_val_epoch(runner, metrics)
        ...
        checkpoint_hook = CheckpointHook(interval=2, by_epoch=True, save_best="loss")
        checkpoint_hook.before_train(runner)
        metrics["loss"] = 0.8
        checkpoint_hook.after_val_epoch(runner, metrics)
        metrics["loss"] = 0.5
        ...
        checkpoint_hook = CheckpointHook(interval=2, by_epoch=True, save_best="acc", rule="less")
        checkpoint_hook.before_train(runner)
        metrics["acc"] = 0.3
        checkpoint_hook.after_val_epoch(runner, metrics)
        ...
        # # no matter what `save_best` is
        checkpoint_hook = CheckpointHook(interval=2, by_epoch=True, save_best="loss", rule="greater")
        checkpoint_hook.before_train(runner)
        metrics["loss"] = 1.0
        checkpoint_hook.after_val_epoch(runner, metrics)
        ...
        checkpoint_hook = CheckpointHook(interval=2, save_best=["acc", "mIoU"], rule="greater")
        ...
        checkpoint_hook = CheckpointHook(interval=2, save_best=["FID", "IS"], rule=["less", "greater"])
        ...
        self.assertEqual(checkpoint_hook.key_indicators, ["acc", "mIoU"])
        self.assertEqual(checkpoint_hook.rules, ["greater", "greater"])
        runner.message_hub = MessageHub.get_instance("test_after_val_epoch_save_multi_best")
        checkpoint_hook.before_train(runner)
        metrics = dict(acc=0.5, mIoU=0.6)
        checkpoint_hook.after_val_epoch(runner, metrics)
        best_acc_name = "best_acc_epoch_9.pth"
        best_acc_path = checkpoint_hook.file_client.join_path(checkpoint_hook.out_dir, best_acc_name)
        best_mIoU_name = "best_mIoU_epoch_9.pth"
        best_mIoU_path = checkpoint_hook.file_client.join_path(checkpoint_hook.out_dir, best_mIoU_name)
        ...
        self.assertEqual(runner.message_hub.get_info("best_ckpt_acc"), best_acc_path)
        self.assertEqual(runner.message_hub.get_info("best_ckpt_mIoU"), best_mIoU_path)
        ...
        metrics = {"acc": 0.5, "map": 0.3}
        checkpoint_hook = CheckpointHook(interval=2, by_epoch=False, save_best="acc", rule="greater")
        checkpoint_hook.before_train(runner)
        checkpoint_hook.after_val_epoch(runner, metrics)
        self.assertEqual(checkpoint_hook.key_indicators, ["acc"])
        self.assertEqual(checkpoint_hook.rules, ["greater"])
        ...
        checkpoint_hook = CheckpointHook(interval=2, by_epoch=False, save_best=["acc", "mIoU"])
        checkpoint_hook.before_train(runner)
        metrics = dict(acc=0.5, mIoU=0.6)
        checkpoint_hook.after_val_epoch(runner, metrics)
        ...
        self.assertFalse(osp.isfile(osp.join(runner.work_dir, "last_checkpoint")))

        # There should only one best checkpoint be reserved
        # dist backend
        for by_epoch, cfg in [(True, self.epoch_based_cfg), (False, self.iter_based_cfg)]:
            self.clear_work_dir()
            cfg = copy.deepcopy(cfg)
            runner = self.build_runner(cfg)
            checkpoint_hook = CheckpointHook(interval=2, by_epoch=by_epoch, save_best="acc")
            checkpoint_hook.before_train(runner)
            checkpoint_hook.after_val_epoch(runner, metrics)
            all_files = os.listdir(runner.work_dir)
            best_ckpts = [file for file in all_files if file.startswith("best")]
        ...
        for by_epoch, cfg in [(True, self.epoch_based_cfg), (False, self.iter_based_cfg)]:
            self.clear_work_dir()
            with (
                patch.dict(sys.modules, {"petrel_client": petrel_client}),
                ...
                patch("mmengine.fileio.backends.PetrelBackend.isfile") as isfile,
            ):  # noqa: E501
                cfg = copy.deepcopy(cfg)
                runner = self.build_runner(cfg)
                metrics = dict(acc=0.5)
                petrel_client.client.Client = MagicMock(return_value=petrel_client)
                checkpoint_hook = CheckpointHook(
                    interval=2, by_epoch=by_epoch, save_best="acc", backend_args=dict(backend="petrel")
                )
                checkpoint_hook.before_train(runner)
                checkpoint_hook.after_val_epoch(runner, metrics)
    ...
    def test_after_train_epoch(self):
        ...
        self.assertEqual(runner.message_hub.get_info("last_ckpt"), osp.join(cfg.work_dir, "epoch_10.pth"))
    ...
    def test_after_train_iter(self):
        ...
        self.assertEqual(runner.message_hub.get_info("last_ckpt"), osp.join(cfg.work_dir, "iter_10.pth"))
    ...
    def test_with_runner(self, training_type):
        ...
        checkpoint_cfg = dict(type="CheckpointHook", interval=1, by_epoch=training_type == "epoch")
        ...
        for i in range(1, 11):
            self.assertEqual(osp.isfile(osp.join(cfg.work_dir, f"{training_type}_{i}.pth")), i % 2 == 0)

        # save_last=True
        self.assertTrue(osp.isfile(osp.join(cfg.work_dir, f"{training_type}_11.pth")))
        ...
        cfg = copy.deepcopy(common_cfg)
        cfg.param_scheduler = [
            dict(type="LinearLR", start_factor=0.1, begin=0, end=500, by_epoch=training_type == "epoch")
        ]
        runner = self.build_runner(cfg)
        runner.train()
        ckpt = torch.load(osp.join(cfg.work_dir, f"{training_type}_11.pth"))
        ...
        runner = self.build_runner(cfg)
        runner.train()
        self.assertTrue(osp.isfile(osp.join(out_dir, osp.basename(cfg.work_dir), f"{training_type}_11.pth")))
        ...
        runner = self.build_runner(cfg)
        runner.train()
        self.assertTrue(osp.isfile(osp.join(cfg.work_dir, f"{training_type}_9.pth")))
        self.assertTrue(osp.isfile(osp.join(cfg.work_dir, f"{training_type}_10.pth")))
        self.assertTrue(osp.isfile(osp.join(cfg.work_dir, f"{training_type}_11.pth")))
        ...
        ckpt = torch.load(osp.join(cfg.work_dir, f"{training_type}_11.pth"))
        self.assertEqual(ckpt["message_hub"]["runtime_info"]["keep_ckpt_ids"], [9, 10, 11])
        ...
        runner = self.build_runner(cfg)
        runner.train()
        self.assertFalse(osp.isfile(osp.join(cfg.work_dir, f"{training_type}_9.pth")))
        self.assertFalse(osp.isfile(osp.join(cfg.work_dir, f"{training_type}_10.pth")))
        self.assertTrue(osp.isfile(osp.join(cfg.work_dir, f"{training_type}_11.pth")))
        self.assertTrue(osp.isfile(osp.join(cfg.work_dir, f"{training_type}_12.pth")))
        ...
        self.assertEqual(best_ckpt_path, ckpt["message_hub"]["runtime_info"]["best_ckpt"])
        ...
        for i in range(5, 11):
            if (i - 5) % 2 == 1:
                ...
                self.assertTrue(osp.isfile(osp.join(cfg.work_dir, f"{training_type}_{i}.pth")))
        self.clear_work_dir()
