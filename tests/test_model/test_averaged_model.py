...
from mmengine.model import ExponentialMovingAverage, MomentumAnnealingEMA, StochasticWeightAverage
...
class TestAveragedModel(TestCase):
    ...
    def _test_swa_model(self, net_device, avg_device):
        model = torch.nn.Sequential(torch.nn.Conv2d(1, 5, kernel_size=3), torch.nn.Linear(5, 10)).to(net_device)
        ...
        for _i in range(n_updates):
            for p, p_avg in zip(model.parameters(), averaged_params, strict=False):
                p.detach().add_(torch.randn_like(p))
                p_avg += p.detach() / n_updates
            averaged_model.update_parameters(model)

        for p_avg, p_swa in zip(averaged_params, averaged_model.parameters(), strict=False):
    ...
    def test_swa_mixed_device(self):
        if not torch.cuda.is_available():
            return
        model = torch.nn.Sequential(torch.nn.Conv2d(1, 5, kernel_size=3), torch.nn.Linear(5, 10))
        model[0].cuda()
        model[1].cpu()
    ...
    def test_swa_state_dict(self):
        ...
        for p_swa, p_swa2 in zip(averaged_model.parameters(), averaged_model2.parameters(), strict=False):
            assert_allclose(p_swa, p_swa2)
        self.assertTrue(averaged_model.steps == averaged_model2.steps)

    def test_ema(self):
        # test invalid momentum
        with self.assertRaisesRegex(AssertionError, "momentum must be in range"):
            model = torch.nn.Sequential(torch.nn.Conv2d(1, 5, kernel_size=3), torch.nn.Linear(5, 10))
        ...
        for i in range(n_updates):
            ...
            for p, p_avg in zip(model.parameters(), averaged_params, strict=False):
                ...
                if i == 0:
                    ...
                    updated_averaged_params.append((p_avg * (1 - momentum) + p * momentum).clone())
            ema_model.update_parameters(model)
            averaged_params = updated_averaged_params

        for p_target, p_ema in zip(averaged_params, ema_model.parameters(), strict=False):
            assert_allclose(p_target, p_ema)

    def test_ema_update_buffers(self):
        # Test EMA and update_buffers as True.
        model = torch.nn.Sequential(
            torch.nn.Conv2d(1, 5, kernel_size=3), torch.nn.BatchNorm2d(5, momentum=0.3), torch.nn.Linear(5, 10)
        )
        momentum = 0.1

        ema_model = ExponentialMovingAverage(model, momentum=momentum, update_buffers=True)
        averaged_params = [
        ...
        for i in range(n_updates):
            ...
            for p, p_avg in zip(params, averaged_params, strict=False):
                p.detach().add_(torch.randn_like(p))
                if i == 0:
                    updated_averaged_params.append(p.clone())
                else:
        ...
        ema_params = [
            param
            for param in itertools.chain(ema_model.module.parameters(), ema_model.module.buffers())
            if param.size() != torch.Size([])
        ]
        for p_target, p_ema in zip(averaged_params, ema_params, strict=False):
            assert_allclose(p_target, p_ema)
    ...
    def test_momentum_annealing_ema(self):
        ...
        ema_model = MomentumAnnealingEMA(model, gamma=gamma, momentum=momentum, update_buffers=True)
        averaged_params = [
            torch.zeros_like(param)
            for param in itertools.chain(model.parameters(), model.buffers())
            if param.size() != torch.Size([])
    ...
    def test_momentum_annealing_ema_with_interval(self):
        ...
        ema_model = MomentumAnnealingEMA(model, gamma=gamma, momentum=momentum, interval=interval, update_buffers=True)
        averaged_params = [
            torch.zeros_like(param)
            for param in itertools.chain(model.parameters(), model.buffers())
            if param.size() != torch.Size([])
