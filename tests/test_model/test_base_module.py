...
class TestBaseModule(TestCase):
    def setUp(self) -> None:
        ...
        self.model_cfg = dict(
            ...
            component4=dict(type="FooLinearConv1d", linear=dict(type="FooLinear"), conv1d=dict(type="FooConv1d")),
        )
    ...
    def test_init_weights(self):
        ...
        assert torch.equal(
            self.model.component1.conv1d.weight, torch.full(self.model.component1.conv1d.weight.shape, 3.0)
        )
        assert torch.equal(self.model.component1.conv1d.bias, torch.full(self.model.component1.conv1d.bias.shape, 4.0))
        assert torch.equal(
            self.model.component2.conv2d.weight, torch.full(self.model.component2.conv2d.weight.shape, 5.0)
        )
        assert torch.equal(self.model.component2.conv2d.bias, torch.full(self.model.component2.conv2d.bias.shape, 6.0))
        assert torch.equal(
            self.model.component3.linear.weight, torch.full(self.model.component3.linear.weight.shape, 1.0)
        )
        assert torch.equal(self.model.component3.linear.bias, torch.full(self.model.component3.linear.bias.shape, 2.0))
        assert torch.equal(
            self.model.component4.linear.linear.weight,
            torch.full(self.model.component4.linear.linear.weight.shape, 1.0),
        )
        assert torch.equal(
            self.model.component4.linear.linear.bias, torch.full(self.model.component4.linear.linear.bias.shape, 2.0)
        )
        assert torch.equal(
        ...
        assert torch.equal(
            self.model.component4.conv1d.conv1d.bias, torch.full(self.model.component4.conv1d.conv1d.bias.shape, 4.0)
        )
        assert torch.equal(self.model.reg.weight, torch.full(self.model.reg.weight.shape, 1.0))
        ...
        class PratrainedModel(FooModel):
            def __init__(
                self, component1=None, component2=None, component3=None, component4=None, init_cfg=None
            ) -> None:
        ...
        # `init_weights`.
        with patch("mmengine.model.base_module.is_model_wrapper", lambda x: isinstance(x, FakeDDP)):
            model = FOOMODELS.build(model_cfg)
...
class TestModuleList(TestCase):
    def test_modulelist_weight_init(self):
        models_cfg = [
            dict(type="FooConv1d", init_cfg=dict(type="Constant", layer="Conv1d", val=0.0, bias=1.0)),
            dict(type="FooConv2d", init_cfg=dict(type="Constant", layer="Conv2d", val=2.0, bias=3.0)),
        ]
        layers = [build_from_cfg(cfg, COMPONENTS) for cfg in models_cfg]
        modellist = ModuleList(layers)
        modellist.init_weights()
        self.assertTrue(torch.equal(modellist[0].conv1d.weight, torch.full(modellist[0].conv1d.weight.shape, 0.0)))
        self.assertTrue(torch.equal(modellist[0].conv1d.bias, torch.full(modellist[0].conv1d.bias.shape, 1.0)))
        self.assertTrue(torch.equal(modellist[1].conv2d.weight, torch.full(modellist[1].conv2d.weight.shape, 2.0)))
        self.assertTrue(torch.equal(modellist[1].conv2d.bias, torch.full(modellist[1].conv2d.bias.shape, 3.0)))
...
class TestModuleDict(TestCase):
    def test_moduledict_weight_init(self):
        models_cfg = dict(
            foo_conv_1d=dict(type="FooConv1d", init_cfg=dict(type="Constant", layer="Conv1d", val=0.0, bias=1.0)),
            foo_conv_2d=dict(type="FooConv2d", init_cfg=dict(type="Constant", layer="Conv2d", val=2.0, bias=3.0)),
        )
        ...
        self.assertTrue(
            torch.equal(
                modeldict["foo_conv_1d"].conv1d.weight, torch.full(modeldict["foo_conv_1d"].conv1d.weight.shape, 0.0)
            )
        )
        self.assertTrue(
            torch.equal(
                modeldict["foo_conv_1d"].conv1d.bias, torch.full(modeldict["foo_conv_1d"].conv1d.bias.shape, 1.0)
            )
        )
        self.assertTrue(
            torch.equal(
                modeldict["foo_conv_2d"].conv2d.weight, torch.full(modeldict["foo_conv_2d"].conv2d.weight.shape, 2.0)
            )
        )
        self.assertTrue(
            torch.equal(
                modeldict["foo_conv_2d"].conv2d.bias, torch.full(modeldict["foo_conv_2d"].conv2d.bias.shape, 3.0)
            )
        )
        # inner init_cfg has higher priority
        layers = {name: build_from_cfg(cfg, COMPONENTS) for name, cfg in models_cfg.items()}
...
class TestSequential(TestCase):
    def test_sequential_model_weight_init(self):
        ...
        self.assertTrue(torch.equal(seq_model[0].conv1d.weight, torch.full(seq_model[0].conv1d.weight.shape, 0.0)))
        self.assertTrue(torch.equal(seq_model[0].conv1d.bias, torch.full(seq_model[0].conv1d.bias.shape, 1.0)))
        self.assertTrue(torch.equal(seq_model[1].conv2d.weight, torch.full(seq_model[1].conv2d.weight.shape, 2.0)))
        self.assertTrue(torch.equal(seq_model[1].conv2d.bias, torch.full(seq_model[1].conv2d.bias.shape, 3.0)))
        # inner init_cfg has higher priority
        layers = [build_from_cfg(cfg, COMPONENTS) for cfg in seq_model_cfg]
