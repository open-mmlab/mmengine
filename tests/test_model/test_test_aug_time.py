...
class TestBaseTTAModel(RunnerTestCase):
    ...
    def test_test_step(self):
        ...
        dict_dataset = [dict(inputs=[1, 2], data_samples=[3, 4]) for _ in range(10)]
        tuple_dataset = [([1, 2], [3, 4]) for _ in range(10)]

        dict_dataloader = DataLoader(dict_dataset, batch_size=2, collate_fn=pseudo_collate)
        tuple_dataloader = DataLoader(tuple_dataset, batch_size=2, collate_fn=pseudo_collate)
    ...
    def test_with_runner(self):
        ...
        cfg.model = dict(type="ToyTestTimeAugModel", module=dict(type="ToyModel"))
