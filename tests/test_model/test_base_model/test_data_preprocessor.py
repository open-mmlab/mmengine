...
class TestImgDataPreprocessor(TestBaseDataPreprocessor):
    def test_init(self):
        ...
        data_processor = ImgDataPreprocessor(
            bgr_to_rgb=True, mean=[0, 0, 0], std=[255, 255, 255], pad_size_divisor=16, pad_value=10
        )
        ...
        assert_allclose(data_processor.std, torch.tensor([255, 255, 255]).view(-1, 1, 1))
        ...
        with self.assertRaisesRegex(AssertionError, "mean and std should be"):
            ImgDataPreprocessor(bgr_to_rgb=True, mean=None, std=[255, 255, 255], pad_size_divisor=16, pad_value=10)

        data_processor = ImgDataPreprocessor(bgr_to_rgb=True, pad_size_divisor=16, pad_value=10)
    ...
    def test_forward(self):
        ...
        for input_, data_sample, target_input, target_data_sample in zip(
            inputs, data_samples, target_inputs, target_data_samples, strict=False
        ):
        ...
        data_preprocessor = ImgDataPreprocessor(mean=(127.5, 127.5, 127.5), std=(127.5, 127.5, 127.5))
        data = dict(inputs=[torch.ones(10, 10), torch.ones(10, 10)], data_sample=None)
        with self.assertRaisesRegex(AssertionError, "If the mean has 3 values"):
        ...
        data_preprocessor = ImgDataPreprocessor(
            mean=(127.5, 127.5, 127.5), std=(127.5, 127.5, 127.5), rgb_to_bgr=True, pad_size_divisor=16
        )
