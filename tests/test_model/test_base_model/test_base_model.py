...
class TestBaseModel(TestCase):
    ...
    def test_parse_losses(self):
        ...
        loss_list = [torch.tensor(2, dtype=torch.float32), torch.tensor(3, dtype=torch.float32)]
    ...
    def test_cuda(self):
        ...
        self.assertEqual(model.toy_model.data_preprocessor._device, torch.device("cpu"))
        model.cuda()
        self.assertEqual(model.data_preprocessor._device, torch.device("cuda"))
        self.assertEqual(model.toy_model.data_preprocessor._device, torch.device("cuda"))
    ...
    def test_to_device(self, device):
        ...
        self.assertTrue(
            all(p.device.type == torch.device(device).type for p in model.parameters())
    ...
    @parameterized.expand(list_product(devices_to_test, dtypes_to_test, ["args", "kwargs", "hybrid"]))
    def test_to_device_and_dtype(self, device, dtype, mode):
        ...
        self.assertTrue(
            ...
            and model.data_preprocessor._device == torch.device(device)
            and all(p.device.type == torch.device(device).type for p in model.parameters())
        )
