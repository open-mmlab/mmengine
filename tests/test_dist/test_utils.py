...
class TestUtils(TestCase):
    ...
    def test_get_data_device(self):
        ...
        with self.assertRaisesRegex(TypeError, "data should be a Tensor, sequence of tensor or dict"):
            dist.get_data_device("123")

    @unittest.skipIf(torch.cuda.device_count() == 0, reason="at lest need 1 gpu to test")
    def test_cast_data_device(self):
        ...
        output = dist.cast_data_device(data, expected_device, out=out)
        for item1, item2 in zip(output, out, strict=False):
        ...
        with self.assertRaisesRegex(ValueError, "length of data and out should be same"):
        ...
        self.assertTrue(torch.allclose(output["key2"][0].cpu(), out["key2"][0]))

        # data is not a valid type
        with self.assertRaisesRegex(TypeError, "data should be a Tensor, list of tensor or dict"):
        ...
        data = torch.tensor([0, 1])
        out = "123"
        with self.assertRaisesRegex(TypeError, "out should be the same type with data"):
            dist.cast_data_device(data, expected_device, out=out)
...
class TestUtilsWithGLOOBackend(MultiProcessTestCase):
    ...
    def test_local_rank(self):
        self._init_dist_env(self.rank, self.world_size)
        self.assertEqual(torch_dist.get_rank(dist.get_local_group()), dist.get_local_rank())
...
class TestUtilsWithNCCLBackend(MultiProcessTestCase):
    ...
    def test_get_data_device(self):
        ...
        # data is a list of Tensor
        data = [torch.tensor([0, 1]).to(expected_device), torch.tensor([2, 3]).to(expected_device)]
        self.assertEqual(dist.get_data_device(data), expected_device)
        ...
        # data is a list containing Tensor and a dict
        data = [torch.tensor([0, 1]).to(expected_device), {"key": torch.tensor([2, 3]).to(expected_device)}]
        self.assertEqual(dist.get_data_device(data), expected_device)
        ...
        # data is a dict
        data = {"key1": torch.tensor([0, 1]).to(expected_device), "key2": torch.tensor([0, 1]).to(expected_device)}
        self.assertEqual(dist.get_data_device(data), expected_device)
        ...
        data = {"key1": torch.tensor([0, 1]), "key2": torch.tensor([0, 1]).to(expected_device)}
        with self.assertRaises(ValueError):
            dist.get_data_device(data)
