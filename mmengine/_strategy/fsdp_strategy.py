...
class FSDPStrategy(DDPStrategy):
    ...
    def __init__(self,
        ...
        if not isinstance(skip_init_weights, bool):
            raise TypeError(
                'skip_init_weights must be a boolean, but got '
                f'{type(skip_init_weights)}')
        self.skip_init_weights = skip_init_weights
        self.model_wrapper = model_wrapper
        super().__init__(
            model_wrapper=model_wrapper, **kwargs)
    ...
    def wrap_model(self, model) -> None:
        ...
        model.set_state_dict_type(model,
    ...
    def load_checkpoint(self, filename, *args, **kwargs) -> Optional[Dict]:
        ...
        if self._is_full_state_dict():
            return super(DDPStrategy, self).load_checkpoint(
    ...
    def load_optim_state_dict(self, state_dict):
        ...
        self.model.optim_state_dict_to_load(
            opitm_state_dict, self.model, self.optim_state_dict)
    ...
    def build_optim_wrapper(
        ...
        if isinstance(optim_wrapper, OptimWrapper):
            ...
            for group in param_groups:
                new_group = {
                    key: value for key, value in group.items() if key != 'param'}
                new_group['params'] = [
            ...
            try:
                ...
                raise RuntimeError(
                    'Failed to convert the built optimizer to the sharded '
                    'optimizer!')
        ...
        for scheduler in schedulers:
            ...
            if isinstance(scheduler, _ParamScheduler):
                ...
                scheduler._last_value = [
                    group[scheduler.param_name] for group in
