...
First, we need to build a **model**. In MMEngine, the model should inherit from `BaseModel`. Besides accepting several parameters from the dataset, its `forward` method needs to accept an extra argument called `mode`:
...
`compute_metrics` accepts a `results` parameter. The input of this parameter is all the information saved in `process` (In the case of a distributed environment, `results` are the information collected from all `process` in all the threads). Using these information to calculate and return a `dict` that holds the results of the evaluation metrics
