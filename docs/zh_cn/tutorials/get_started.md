...
以下根据上图简述这些模块的功能于联系，用户可以通过各个组件的用户文档了解他们。
...
- [模型（Model）](./model.md)：在训练过程中接受数据、输出 loss，在测试、推理任务中接受数据，并进行预测。分布式训练等情况下会被模型的封装器（Model Wrapper，如 DistributedDataParallel）封装一层。
...
- [优化器（Optimizer）](./optimizer.md)：优化器，负责在训练过程中执行反向传播优化模型。
...
- [配置类（Config）](./config.md)：在 OpenMMLab 算法库中，用户可以通过编写 config 来配置训练、测试过程
