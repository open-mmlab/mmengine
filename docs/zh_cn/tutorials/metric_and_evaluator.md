...
**在线评测**：测试数据通常会被划分为若干批次（batch）。通过一个循环，依次将每个批次的数据送入模型，得到对应的预测结果，并将测试数据和模型预测结果送入评测指标的 `process()` 方法进行处理。当循环结束后，调用评测指标的 `evaluate()` 方法，即可计算得到对应的评测指标。

**离线评测**：与在线评测过程类似，区别是直接读取预先保存的模型预测结果来进行评测。为了避免同时处理大量数据导致内存溢出，离线评测时会将测试数据和预测结果分成若干个块（Chunk）进行处理，类似在线评测中的批次。

<div align="center">
    <img src="https://user-images.githubusercontent.com/15977946/163719822-af68b1e9-b74c-414c-8b29-303bf9bed5e7.jpg" width="500"/>
</div>

## 在配置文件中配置评测指标

在配置文件中可以指定模型验证、测试阶段使用的评测指标及其参数。例如，用户在训练分类模型时，希望在模型验证阶段使用分类正确率和 F1 Score 两个评测指标，可以按以下方式配置：
...
如果只使用单个评测指标，也可以省略掉配置中的列表，直接指定评测指标参数。例如，在模型测试阶段使用 COCO 评测指标，对应的配置如下：

```python
test_evaluator = dict(type='COCO')
```
