...
如果安装过程被中断，可以重复执行 `uv run pre-commit run --all-files` 继续安装。
...
yapf 和 isort 的配置可以在 [pyproject.toml](https://github.com/open-mmlab/mmengine/blob/main/pyproject.toml) 找到
