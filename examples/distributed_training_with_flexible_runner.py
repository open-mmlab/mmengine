...
def main():
    ...
    if args.use_deepspeed:
        ...
        optim_wrapper = dict(type="DeepSpeedOptimWrapper", optimizer=dict(type="AdamW", lr=1e-3))
    elif args.use_fsdp:
        from functools import partial

        from torch.distributed.fsdp.wrap import size_based_auto_wrap_policy

        size_based_auto_wrap_policy = partial(size_based_auto_wrap_policy, min_num_params=1e7)
        strategy = dict(type="FSDPStrategy", model_wrapper=dict(auto_wrap_policy=size_based_auto_wrap_policy))
        optim_wrapper = dict(type="AmpOptimWrapper", optimizer=dict(type="AdamW", lr=1e-3))
