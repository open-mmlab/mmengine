...
class MMT5ForTranslation(BaseModel):
    ...
    def forward(self, label, input_ids, attention_mask, mode):
        if mode == "loss":
            output = self.model(input_ids=input_ids, attention_mask=attention_mask, labels=label)
...
class Accuracy(BaseMetric):
    def process(self, data_batch, data_samples):
        ...
        prediction_lens = torch.tensor(
            [torch.count_nonzero(pred != tokenizer.pad_token_id) for pred in outputs], dtype=torch.float64
        )
...
def collate_fn(data):
    ...
    label[label == tokenizer.pad_token_id] = -100  # ignore contribution to loss
    return dict(label=label, input_ids=input_dict.input_ids, attention_mask=input_dict.attention_mask)
...
def main():
    ...
    train_loader = dict(
        batch_size=16, dataset=train_set, sampler=dict(type="DefaultSampler", shuffle=True), collate_fn=collate_fn
    )
    test_loader = dict(
